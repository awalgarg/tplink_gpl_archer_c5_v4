...
#define MARK_SET8021P '6' /* zl added 2013-1-6 */
#define MARK_8021PAUTOSET '7' /* zl added 2013-1-6 */
static struct option opts[] =
{
	...
	{ "8021p-set"   , required_argument, 0, MARK_SET8021P }, /* zl added 2013-1-6 */
	{ "ftos-8021p"  , required_argument, 0, MARK_8021PAUTOSET }, /* zl added 2013-1-6 */
	{ 0 }
};
...
static void print_help()
{
	...
	" --or-mark  value     : Or nfmark with value (nfmark |= value)\n"
	" --mark-and value     : And nfmark with value (nfmark &= value)\n"
	" --mark-xor value     : Xor nfmark with value (nfmark ^= value)\n"
	" --8021p-set value    : Set vlan tag's 802.1P value\n"   /* zl added 2013-1-6 */
	" --ftos-8021p value   : Set vlan tag's 802.1P value according to DSCP value"  /* zl added 2013-1-6 */
...
#define OPT_MARK_SET8021P 0x20 /* zl added 2013-1-6 */
#define OPT_MARK_8021PAUTOSET 0x40 /* zl added 2013-1-6 */
   ...
   struct ebt_entry_target **target)
{
	...
	case MARK_SETMARK:
		...
		if (*flags & (OPT_MARK_ORMARK|OPT_MARK_ANDMARK|OPT_MARK_XORMARK|OPT_MARK_SET8021P|OPT_MARK_8021PAUTOSET))
			ebt_print_error2("--mark-set cannot be used together with specific --mark option");
                break;
	case MARK_ORMARK:
		ebt_check_option2(flags, OPT_MARK_ORMARK);
		if (*flags & (OPT_MARK_SETMARK|OPT_MARK_ANDMARK|OPT_MARK_XORMARK|OPT_MARK_SET8021P|OPT_MARK_8021PAUTOSET))
	...
	case MARK_ANDMARK:
		...
		if (*flags & (OPT_MARK_SETMARK|OPT_MARK_ORMARK|OPT_MARK_XORMARK|OPT_MARK_SET8021P|OPT_MARK_8021PAUTOSET))
	...
	case MARK_XORMARK:
		...
		if (*flags & (OPT_MARK_SETMARK|OPT_MARK_ANDMARK|OPT_MARK_ORMARK|OPT_MARK_SET8021P|OPT_MARK_8021PAUTOSET))
    ...
    /* zl added 2013-1-6 */                
    case MARK_SET8021P:
        ebt_check_option2(flags, OPT_MARK_SET8021P);
        if (*flags & (OPT_MARK_SETMARK|OPT_MARK_ANDMARK|OPT_MARK_ORMARK|OPT_MARK_XORMARK|OPT_MARK_8021PAUTOSET))
            ebt_print_error2("--8021p-set cannot be used together with specific --mark option");
        markinfo->target = (markinfo->target & EBT_VERDICT_BITS) | VLAN_8021P_SET_VALUE;
                break;
    case MARK_8021PAUTOSET:
        ebt_check_option2(flags, OPT_MARK_8021PAUTOSET);
        if (*flags & (OPT_MARK_SETMARK|OPT_MARK_ANDMARK|OPT_MARK_ORMARK|OPT_MARK_XORMARK|OPT_MARK_SET8021P))
            ebt_print_error2("--ftos-8021p cannot be used together with specific --mark option");
        markinfo->target = (markinfo->target & EBT_VERDICT_BITS) | VLAN_8021P_AUTO_SET;
                break;
    /* end--added */
	 default:
		return 0;
   ...
   const struct ebt_entry_target *target)
{
	...
	if (tmp == MARK_SET_VALUE)
		...
		printf("--mark-and");
	else if (tmp == VLAN_8021P_SET_VALUE)
	    printf("--8021p-set");
	else if (tmp == VLAN_8021P_AUTO_SET)
	    printf("--ftos-8021p");
	else
