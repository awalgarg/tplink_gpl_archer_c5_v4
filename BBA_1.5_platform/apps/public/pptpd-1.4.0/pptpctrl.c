...
/* added by CCy for controling ppp unit, 2015-08-24 */
#define PPTPD_PPP_UNIT_STATUS_FILE		"/var/pptpd/pppUnit.status"
#define	PPTPD_PPP_UNIT_NUM_BASE			(20)
typedef enum
{
	ACT_PPTPD_PPP_UNIT_RELEASE 	= 0,
	ACT_PPTPD_PPP_UNIT_OCCUPY  = 1,
}ACT_PPTPD_PPP_UNIT_OPT;
typedef enum
{
	ACT_PPTPD_PARENT_CALL	= 0,
	ACT_PPTPD_CHILD_CALL	= 1, 
}ACT_PPTPD_WHICH_CALL;

int		curIfUnit = -1;	/* curIfUnit starts from 20 to 29, 10 clients maximum */
static int findAvailablePppUnit(void);
static int setPppUnitStatus(ACT_PPTPD_PPP_UNIT_OPT setOpt, ACT_PPTPD_WHICH_CALL call);
/* add ended by CCy */
...
int main(int argc, char **argv)
{
		...
		/* added by CCy for controling ppp unit, 2015-08-24 */
		setPppUnitStatus(ACT_PPTPD_PPP_UNIT_RELEASE, ACT_PPTPD_PARENT_CALL);
		/* add ended by CCy */
...
/* In case we move clientSocket back off stdin */
				...
				/* added by CCy for controling ppp unit, 2015-08-24 */
				/* any early exit of program pptpctrl after launch_pppd() must call setPppUnitStatus() */
				setPppUnitStatus(ACT_PPTPD_PPP_UNIT_RELEASE, ACT_PPTPD_CHILD_CALL);
				/* add ended by CCy */
                _exit(1);
        }
...
static void launch_pppd(char **pppaddrs, struct in_addr *inetaddrs)
{
		...
		/* added by CCy for controling ppp unit, 2015-08-24 */
		if (PPTPD_PPP_UNIT_NUM_BASE <= findAvailablePppUnit())
		{
			sprintf(ifUnitStr, "%d", curIfUnit);
			pppd_argv[an++] = "unit";
			pppd_argv[an++] = ifUnitStr;
			setPppUnitStatus(ACT_PPTPD_PPP_UNIT_OCCUPY, ACT_PPTPD_CHILD_CALL);
		}
		/* add ended by CCy */
...
static int findAvailablePppUnit(void)
{
	...
	if (NULL == (pFileFd = fopen(PPTPD_PPP_UNIT_STATUS_FILE, "r")))
	{
		syslog(LOG_ERR, "CTRL: %s: Failed to open %s. %s", __FUNCTION__, PPTPD_PPP_UNIT_STATUS_FILE,
				strerror(errno));
		return -1;
	}

	if (NULL != fgets(tmpBuf, sizeof(tmpBuf), pFileFd))
	{
		if (10 != sscanf(tmpBuf, "%d %d %d %d %d | %d %d %d %d %d # ",
						&u[0], &u[1], &u[2], &u[3], &u[4], &u[5], &u[6], &u[7], &u[8], &u[9]))
		{
			syslog(LOG_ERR, "CTRL: %s: Match error.", __FUNCTION__);
			fclose(pFileFd);
			return -1;
		}
		for (index = 0; index < 10; index++)
		{
			if (0 == u[index])
			{
				curIfUnit = PPTPD_PPP_UNIT_NUM_BASE + index;
				fclose(pFileFd);
				return curIfUnit;
			}
		}
		syslog(LOG_ERR, "CTRL: %s: Failed to find available ppp unit.", __FUNCTION__);
	}
	else
	{
		syslog(LOG_ERR, "CTRL: %s: Fgets() failed", __FUNCTION__);
	}
...
static int setPppUnitStatus(ACT_PPTPD_PPP_UNIT_OPT setOpt, ACT_PPTPD_WHICH_CALL call)
{
	...
	int			index					= 0;
	pid_t		pid;
	char		pidPppUnitFileName[35]  = {0};
	char		cmd[40]					= {0};

	if (ACT_PPTPD_PPP_UNIT_OCCUPY == setOpt)
	{
		/*	this branch is always called from child process since only launch_pppd() will call 
			ACT_PPTPD_PPP_UNIT_OCCUPY*/
		pid = getpid();
		sprintf(pidPppUnitFileName, "/var/pptpd/%d.pppUnit", pid);
		if (NULL == (pFileFd = fopen(pidPppUnitFileName, "w")))
		{
			syslog(LOG_ERR, "CTRL: %s: Failed to open %s. %s", __FUNCTION__, pidPppUnitFileName,
					strerror(errno));
			return -1;
		}
		fprintf(pFileFd, "%d", curIfUnit);
		fclose(pFileFd);
	}
	else
	{
		/* in parent process, will have to read out the value first, only after that can we delete it */
		if (ACT_PPTPD_PARENT_CALL == call)
		{
			sprintf(pidPppUnitFileName, "/var/pptpd/%d.pppUnit", pppfork);
			if (NULL == (pFileFd = fopen(pidPppUnitFileName, "r")))
			{
				syslog(LOG_ERR, "CTRL: %s: Failed to open %s. %s", __FUNCTION__, pidPppUnitFileName,
						strerror(errno));
				return -1;
			...
			if (NULL != fgets(tmpBuf, sizeof(tmpBuf), pFileFd))
			{
				if (1 != sscanf(tmpBuf, "%d", &curIfUnit))
				{
					syslog(LOG_ERR, "CTRL: %s: Match error.", __FUNCTION__);
					fclose(pFileFd);
					return -1;					
		...
		sprintf(cmd, "rm %s", pidPppUnitFileName);
		system(cmd);
	}

	syslog(LOG_DEBUG, "CTRL: %s: curIfUnit is %d.", __FUNCTION__, curIfUnit);
	if (NULL == (pFileFd = fopen(PPTPD_PPP_UNIT_STATUS_FILE, "r")))
	{
		syslog(LOG_ERR, "CTRL: %s: Failed to open %s. %s", __FUNCTION__, PPTPD_PPP_UNIT_STATUS_FILE,
				strerror(errno));
	...
	if (PPTPD_PPP_UNIT_NUM_BASE <= curIfUnit) 
	{
		u[curIfUnit-PPTPD_PPP_UNIT_NUM_BASE] = setOpt;
		syslog(LOG_DEBUG, "CTRL: %s: Set u[%d] to %d.", __FUNCTION__, curIfUnit-PPTPD_PPP_UNIT_NUM_BASE, setOpt);
		if (ACT_PPTPD_PPP_UNIT_RELEASE == setOpt)
		{
			curIfUnit = -1;
		}
	}
	if (NULL == (pFileFd = fopen(PPTPD_PPP_UNIT_STATUS_FILE, "w")))
	{
		syslog(LOG_ERR, "CTRL: %s: Failed to open %s", __FUNCTION__);
		return -1;
	}

	/* 10 clients maximum, must change accordingly */
	fprintf(pFileFd, "%d %d %d %d %d | %d %d %d %d %d # ",
					u[0], u[1], u[2], u[3], u[4], u[5], u[6], u[7], u[8], u[9]);
	fclose(pFileFd);
	return 0;	
}
