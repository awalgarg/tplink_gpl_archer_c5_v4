...
send_ra(int sock, struct Interface *iface, struct in6_addr *dest)
{
	...
	radvert->nd_ra_router_lifetime	 = (!check_ip6_forwarding() && check_ip6_dftroute()) ? htons(iface->AdvDefaultLifetime) : 0;

	/*  If the Router Lifetime is zero, the preference value MUST be set to Medium(00) by the
	 *	sender and MUST be ignored by the receiver, RFC4191 2.2 Section.
	 */
	if (radvert->nd_ra_router_lifetime == 0)
	{
		radvert->nd_ra_flags_reserved   |= (0 << ND_OPT_RI_PRF_SHIFT) & ND_OPT_RI_PRF_MASK;
	}
	else
	{
		radvert->nd_ra_flags_reserved   |=
...
static int check_ip6_dftroute(void)
{
	...
	while (5 == fscanf(fp, "%s%lx%*s%*s%s%lx%*s%*s%*s%s", dstAddr, &plen, dftGwAddr, &metric, intfaceName))
	{	
		if ((0 == strcmp(dstAddr, IPV6_ROUTE_ANY_ADDRESS)) &&
			(0 == plen) &&
			(0 != strcmp(intfaceName, "lo")) &&
			(IPV6_ROUTE_METRIC_DEFAULT == metric))
		{
...
static int check_ip6_dftroute(void)
{
	dlog(LOG_DEBUG, 4, "checking ipv6 default route not supported");
	return 1;
}
#endif /* __linux__ */