 ...
 * @path: path to driver directory (通向驱动的sysfs文件系统直接路径，如/sys/bus/usb/drivers/usbip)
 * returns struct sysfs_driver with success and NULL with error
 */
struct sysfs_driver *sysfs_open_driver_path(const char *path)
{
	...
	if (sysfs_get_name_from_path(path, driver->name, SYSFS_NAME_LEN)) {	/* driver->name == usbip */
