 ...
 * @mnt_path: place to put "sysfs" mount point (用于存放sysfs文件系统挂载点路径的第一个目录名)
 * @len: size of mnt_path (参数mnt_path用于存放字符串的长度)
 * returns 0 with success and -1 with error.
 * 
 *如函数名，获取sysfs文件系统挂载点路径的第一个目录名，存放到第一个参数mnt_path中；
 *挂载点路径是通过环境变量SYSFS_PATH 得到的，一般为"/sys",如果环境变量SYSFS_PATH 未定义，
 *默认为"/sys"；只有当参数mnt_path为空或者参数len==0，才会返回-1错误。
 */
int sysfs_get_mnt_path(char *mnt_path, size_t len)
{
	...
	if (sysfs_path[0] == '\0') {
		/* possible override of real mount path */
		sysfs_path_env = getenv(SYSFS_PATH_ENV); /* 获取环境变量SYSFS_PATH 的名称: "/sys" */
 ...
 * @len: size of name
 */
int sysfs_get_name_from_path(const char *path, char *name, size_t len)	/* 获取路径的最后一个，即文件名 */
{
