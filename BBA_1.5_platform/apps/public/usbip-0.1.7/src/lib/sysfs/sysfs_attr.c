...
struct sysfs_attribute *sysfs_open_attribute(const char *path)
{
	...
	if ((stat(sysattr->path, &fileinfo)) != 0) {
		...
		if (fileinfo.st_mode & S_IRUSR)	/* 文件所有者具有读取权限 */
			sysattr->method |= SYSFS_METHOD_SHOW;
		if (fileinfo.st_mode & S_IWUSR)	/* 文件所有者具有写入权限 */
...
int sysfs_read_attribute(struct sysfs_attribute *sysattr)
{
	...
	pgsize = getpagesize();	/* 返回系统分页大小(Byte) */
	fbuf = (char *)calloc(1, pgsize+1);
	if (!fbuf) {
		dprintf("calloc failed\n");
		return -1;
	}
	if ((fd = open(sysattr->path, O_RDONLY)) < 0) {	/* open file read only */
		dprintf("Error reading attribute %s\n", sysattr->path);
		free(fbuf);
		return -1;
	}
	length = read(fd, fbuf, pgsize);	/* read fd (pgsize bytes) to fbuf */
	if (length < 0) {
		...
		const char *new_value, size_t len)
{
	...
	length = write(fd, new_value, len);	/* write sockfd to usbip_sockfd */
	if (length < 0) {
...
struct dlist *read_dir_links(const char *path)
{
	...
	dir = opendir(path);	/* read  "path" , return dir stream */
	if (!dir) {
	...
	while ((dirent = readdir(dir)) != NULL) {
		...
		safestrcpy(file_path, path);	/* file_path = "/sys/bus/usb/drivers/usbip"  */
		safestrcat(file_path, "/");
		safestrcat(file_path, dirent->d_name);	/* file_path = "/sys/bus/usb/drivers/usbip/bind , ......"  */
