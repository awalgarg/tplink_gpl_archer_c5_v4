...
#define SINGLE_PVC_MULTI_CONNECT 1
...
void list_vcc(void)
{
	...
	if (lastsock<0) {
		...
		err = ioctl (lastsock, ATM_SHOWBACKENDIF, NULL);
	}
...
#ifdef SINGLE_PVC_MULTI_CONNECT
int create_br(int itfnum, int payload, int ext, int del)
#else
...
#ifdef SINGLE_PVC_MULTI_CONNECT
  	create_br(params->itfnum, params->payload, params->ext, params->del);
#else
  	create_br(params->itfnum, params->payload);
#endif

#ifdef SINGLE_PVC_MULTI_CONNECT
  	if ((params->ext > 0)||(params->del > 0))
  	{
  		return ;
  	}
#endif
...
void usage(char *s)
{
  printf("usage: %s [-b] [[-c number] [-e 0|1|2|3] [-t ubr|cbr|vbr] [-p pcr] [-q scr] [-m mbs] [-a [itf.]vpi.vci", s);
#ifdef SINGLE_PVC_MULTI_CONNECT
  printf(" [-x/d number]");	
  printf("[-l]");
#endif
...
int get_vpi_vci(char * p_str)
{
	...
	sprintf(cmd, "echo vpi = %d vci = %d > /var/run/vpi_%d_vci_%d", vpi, vci, vpi, vci); 
	system(cmd);
	return 0;
}
#endif
...
{
  int c, background=0;
  /* added by yangxv for record process pid£¬2009.02.23 */
  char cmd[128];
  /* end added */
  ...
  if (argc>1)
    while ((c = getopt(argc, argv,"a:bc:d:e:t:p:q:m:s:x:?h")) !=EOF)
      switch (c) {
      case 'a':
		params.astr=optarg;
#ifdef SINGLE_PVC_MULTI_CONNECT
		//printf("zxmdebug parse vpi vci\n");
  		//get_vpi_vci(optarg);
#endif		
	    break;
      case 'b':
...
#endif
  sprintf(cmd, "echo %d > /var/run/nas%d.pid", pid, params.itfnum); 
