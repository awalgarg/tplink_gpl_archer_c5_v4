...
//add by chz to log the debug message to file.
#define MAX_LOG_BUFFER_LEN 256
void log_file_local(char *log)
{
  FILE *fd_log = NULL;
  int len = 0;
  pid_t pid = 0;
  char buf_log[MAX_LOG_BUFFER_LEN];
  ...
  len = strlen(log);
  if (len == 0 || (len > MAX_LOG_BUFFER_LEN - strlen("PID XXXX: ")))
  {
  	return;
  }
	
  fd_log = fopen("/var/vsftp/log/debug_log", "a");
  if (fd_log == NULL)
  {
  	return;
  }
  ...
  pid = getpid();
  sprintf(buf_log, "PID %d: %s", pid, log);
  len = strlen(buf_log);
  fwrite(buf_log, 1, len, fd_log);
  fclose(fd_log);
}
//end add
