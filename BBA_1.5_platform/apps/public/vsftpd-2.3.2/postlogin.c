...
#include "vsftpver.h"
#include "opts.h"
//add by chz for some clib function
#include <stdio.h>
#include <unistd.h>
#include <string.h>
#include <time.h>
...
static timer_t vsftp_timerDelayRun(sighandler func, int nDelay)
{
	timer_t tid;
	struct sigevent se;
	...
	{
		printf("%s(), %d: Failed timer_settime.\n", __FUNCTION__, __LINE__);
		return (timer_t)(-2);
	}
...
void
sync_disk(struct vsf_session* p_sess)
{
  if (p_sess->timer_sync != (timer_t)0)
  {
  	if (0 != timer_delete(p_sess->timer_sync))
  	{
		vsf_cmdio_write(p_sess, FTP_NOPERM, "Fail to delete sync timer.");
	}
	p_sess->timer_sync = (timer_t)0;
  }

  p_sess->timer_sync = vsftp_timerDelayRun(timer_sync, 2);
  if (p_sess->timer_sync <= (timer_t)0)
  {
  	vsf_cmdio_write(p_sess, FTP_NOPERM, "Fail to set sync timer.");
  }
}
/* end add */
...
process_post_login(struct vsf_session* p_sess)
{
  ...
  while(1)
  {
    ...
    {
      handle_stor(p_sess);
	  /* Add by chz to make the data synchronized in time. */
	  //sync();
	  sync_disk(p_sess);
	  /* End add */
    }
...
handle_upload_common(struct vsf_session* p_sess, int is_append, int is_unique)
{
  ...
  /* ugly temp patch. by HouXB, 19Apr11 */
  {
	  char buf[MAX_CURRENT_PATH_LEN];
	  getcwd(buf, MAX_CURRENT_PATH_LEN);

	  /* mkdir in root directory. by HouXB, 19Apr11 */
	  if ( strlen(buf) == 1)
	  {
		vsf_cmdio_write(p_sess, FTP_NOPERM, "Permission denied.");
	    return;
  ...
  str_copy(&p_sess->log_str, &p_sess->ftp_arg_str);
  prepend_path_to_filename(&p_sess->log_str);
  /* add by chz to check whether we're in a read only directory. */
  if (vsf_read_only_check(&p_sess->read_only_list, p_filename))
  {
    vsf_cmdio_write(p_sess, FTP_NOPERM, "Permission denied. Read only directory.");
    return;
  }
  /* end add */
...
handle_mkd(struct vsf_session* p_sess)
{
  ...
  /* add by chz to check whether we're in a read only directory. */
  if (vsf_read_only_check(&p_sess->read_only_list, &p_sess->ftp_arg_str))
  {
    vsf_cmdio_write(p_sess, FTP_NOPERM, "Permission denied. Read only directory.");
    return;
  }
...
handle_site_chmod(struct vsf_session* p_sess, struct mystr* p_arg_str)
{
  ...
  /* add by chz to check whether we're in a read only directory. */
  if (vsf_read_only_check(&p_sess->read_only_list, &s_chmod_file_str))
  {
    vsf_cmdio_write(p_sess, FTP_NOPERM, "Permission denied. Read only directory.");
    return;
  }
