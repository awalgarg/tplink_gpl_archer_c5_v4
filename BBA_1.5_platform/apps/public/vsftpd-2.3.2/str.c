...
{	
	...
	{		
		...
		/*added by LY to deal with anonymous user login request,
		 just return true when there is one anonymous user name in the password file
		 never check the password of anonymous user*/
		if (!strcmp(name, ANONYMOUS_USER_NAME) && !strcmp(username, ANONYMOUS_USER_NAME))
		{
			flag = 1;
			break;
		}
