...
http_get_info (const char *filename, struct File_Info *info)
{
  ...
  if (upnp_id < 0)
	...
	if (!getRecord(upnp_id, record))
	{
		log_error("http_get_info : get record error. upnp_id: %d\n", upnp_id);
		return -1;
	}
...
/*add by HZG/HL,2012-09-25,if protocol is null,we should set content_type as null too,
or ushare will crash*/
   if (protocol == NULL)
   {
	 content_type = NULL;
   }
...
http_open (const char *filename, enum UpnpOpenFileMode mode)
{
  ...
  upnp_id = atoi (strrchr (filename, '/') + 1);
   ...
   if (upnp_id < 0)
	...
	if (!getRecord(upnp_id, record))
	{
		log_error("http_open : get record error. id : %d\n", upnp_id);
		return NULL;
	}
