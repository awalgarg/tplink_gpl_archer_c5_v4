...
/* 
 * fn		PJ_INLINE(char*) pj_strnull( const pj_str_t* str)
 * brief	On mips platform, when we call the print functions, such as printf,
 *			snprintf, and so on, and format is "%.*s", if the specified string
 *			pointer is null, the string output is "(null)". This will result in
 *			many problem. So, when the string pointer is null, we replace it with
 *			"".
 * details	
 *
 * param[in]	
 ...
 * note		
 */
PJ_INLINE(char*) pj_strnull( char* str)
{
	return (NULL == str )? "" : str;
}
