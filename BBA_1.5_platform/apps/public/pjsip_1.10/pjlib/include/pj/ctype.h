...
/*yuchuwei@20121015
注意:因为eyebeam和3cxphone转换之后的ASCII码中有关字母部分是大写的，所以pjSIP
也改为大写的。
*/
#define pj_hex_digits_sip_escape "0123456789ABCDEF"
...
/**
 * Convert a value to hex representation. Use upper case character
 * @param value	    Integral value to convert.
 * @param p	    Buffer to hold the hex representation, which must be
 *		    at least two bytes length.
 */
PJ_INLINE(void) pj_val_to_hex_digit_escape(unsigned value, char *p)
{
    *p++ = pj_hex_digits_sip_escape[ (value & 0xF0) >> 4 ];
    *p   = pj_hex_digits_sip_escape[ (value & 0x0F) ];
}
