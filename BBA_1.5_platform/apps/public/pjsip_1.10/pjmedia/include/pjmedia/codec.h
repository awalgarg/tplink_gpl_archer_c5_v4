...
typedef struct pjmedia_codec_param
{
    ...
    struct
    {
       ...
       pjmedia_format_id fmt_id;	/**< Source format, it's format of
					     ...
					     output.			    */
		 /*ycw-pjsip. t38*/
		 pj_uint32_t max_datagram;  /**< Attribute value of a=T38FaxMaxDatagram */
		 pj_uint32_t max_bitrate; /*Attribute value of a=T38MaxBitRate*/
		 pj_bool_t	enableUdpEc;	/*If present Attribute a=T38FaxUdpEC, it is true,
		 									or it is false*/
    } info;
...
/* 
 * brief	ycw-pjsip. To check the codec exists in the codec manager or not.
 */
pj_status_t pjmedia_codec_mgr_codec_is_exist( const pjmedia_codec_mgr* mgr, const pj_str_t* codec);

/* 
 * brief	ycw-pjsip. Get the number of the codec manager's codecs.
 */
