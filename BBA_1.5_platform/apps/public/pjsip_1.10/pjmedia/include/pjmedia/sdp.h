...
/**
 * Atheros
 * This structure declares SDP \a gpmd attribute.
 */
struct pjmedia_sdp_gpmd
{
    pj_str_t        pt;        /**< Payload type.*/
    pj_str_t        param;     /**< Parameter.   */
};
...
/**
 * @see pjmedia_sdp_gpmd
 */
typedef struct pjmedia_sdp_gpmd pjmedia_sdp_gpmd;


/**
 * Atheros
 * Get the gpmd representation of the same SDP attribute.
 *
 * @param attr		Generic attribute to be converted to gpmd, which
 *			name must be "gpmd".
 * @param gpmd		SDP gpmd attribute to be initialized.
 *
 * @return		PJ_SUCCESS on success.
 */
PJ_DECL(pj_status_t) pjmedia_sdp_attr_get_gpmd(const pjmedia_sdp_attr *attr,
