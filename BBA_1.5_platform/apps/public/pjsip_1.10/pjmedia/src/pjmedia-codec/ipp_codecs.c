/* $Id: ipp_codecs.c 3553 2011-05-05 06:14:19Z nanang $ */
/* 
...
pj_status_t pjmedia_codec_ipp_init( pjmedia_endpt *endpt )
{
    ...
    /* Register codec factory to endpoint. */
    status = pjmedia_codec_mgr_register_factory(full_codec_mgr, &ipp_factory.base);
    if (status != PJ_SUCCESS)
    {
		goto on_error;
    }
...
pj_status_t pjmedia_codec_ipp_deinit(void)
{
    ...
    status = pjmedia_codec_mgr_unregister_factory(codec_mgr,
	 ...
	 #else
	 /* Get the codec manager. */
    full_codec_mgr = pjmedia_endpt_get_full_codec_mgr(ipp_factory.endpt);
    if (!full_codec_mgr)
	 {
		pj_pool_release(ipp_factory.pool);
		ipp_factory.pool = NULL;
    ...
    /* Unregister IPP codecs factory. */
    status = pjmedia_codec_mgr_unregister_factory(full_codec_mgr, &ipp_factory.base);
	 #endif
