...
pj_status_t pjmedia_codec_g722_init( pjmedia_endpt *endpt )
{
    ...
    g722_codec_factory.pool = pjmedia_endpt_create_pool(endpt, "g722", 1000, 1000);
	...
	goto on_error;
	 ...
	 status = pjmedia_codec_mgr_register_factory(full_codec_mgr, &g722_codec_factory.base);
	 if (status != PJ_SUCCESS)
	 {
	 	goto on_error;
	 }
...
pj_status_t pjmedia_codec_g722_deinit(void)
{
    ...
    full_codec_mgr = pjmedia_endpt_get_full_codec_mgr(g722_codec_factory.endpt);
    if (!full_codec_mgr)
	 {
		pj_pool_release(g722_codec_factory.pool);
		g722_codec_factory.pool = NULL;
