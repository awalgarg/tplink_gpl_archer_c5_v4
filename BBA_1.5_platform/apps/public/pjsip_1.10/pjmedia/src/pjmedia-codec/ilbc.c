					     ...
					     int mode )
{
    ...
    /* Register codec factory to endpoint. */
    status = pjmedia_codec_mgr_register_factory(full_codec_mgr, &ilbc_factory.base);
    if (status != PJ_SUCCESS)
    {
		return status;
    }
...
pj_status_t pjmedia_codec_ilbc_deinit(void)
{
    ...
    status = pjmedia_codec_mgr_unregister_factory(codec_mgr,
	 ...
	 #else
	 /* Get the codec manager. */
    full_codec_mgr = pjmedia_endpt_get_full_codec_mgr(ilbc_factory.endpt);
    if (!full_codec_mgr)
    {
		return PJ_EINVALIDOP;
    }
    ...
    /* Unregister iLBC codec factory. */
    status = pjmedia_codec_mgr_unregister_factory(full_codec_mgr, &ilbc_factory.base);
	 #endif
