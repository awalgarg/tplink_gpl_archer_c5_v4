/* $Id: g7221.c 3553 2011-05-05 06:14:19Z nanang $ */
/* 
...
pj_status_t pjmedia_codec_g7221_init( pjmedia_endpt *endpt )
{
    ...
    /* Register codec factory to endpoint. */
    status = pjmedia_codec_mgr_register_factory(full_codec_mgr, &codec_factory.base);
    if (status != PJ_SUCCESS)
    {
		goto on_error;
    }
...
pj_status_t pjmedia_codec_g7221_deinit(void)
{
    ...
    status = pjmedia_codec_mgr_unregister_factory(codec_mgr,
	 ...
	 #else
	 /* Get the codec manager. */
    full_codec_mgr = pjmedia_endpt_get_full_codec_mgr(codec_factory.endpt);
    if (!full_codec_mgr)
	 {
	 	pj_mutex_destroy(codec_factory.mutex);
		pj_pool_release(codec_factory.pool);
    ...
    /* Unregister G722.1 codec factory. */
    status = pjmedia_codec_mgr_unregister_factory(full_codec_mgr, &codec_factory.base);
	 #endif
