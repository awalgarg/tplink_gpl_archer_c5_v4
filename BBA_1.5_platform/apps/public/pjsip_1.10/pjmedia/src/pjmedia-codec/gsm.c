...
pj_status_t pjmedia_codec_gsm_init( pjmedia_endpt *endpt )
{
    ...
    /* Register codec factory to endpoint. */
    status = pjmedia_codec_mgr_register_factory(full_codec_mgr, &gsm_codec_factory.base);
    if (status != PJ_SUCCESS)
    {
		goto on_error;
    }
...
pj_status_t pjmedia_codec_gsm_deinit(void)
{
    ...
    */
    ...
    status = pjmedia_codec_mgr_unregister_factory(codec_mgr,
	 ...
	 #else
	     /* Get the codec manager. */
    full_codec_mgr = pjmedia_endpt_get_full_codec_mgr(gsm_codec_factory.endpt);
    if (!full_codec_mgr)
	 {
		pj_pool_release(gsm_codec_factory.pool);
		gsm_codec_factory.pool = NULL;
