    ...
    for (i=0; i<master->desc.fmt_count; ++i)
	 {
		...
		for (j=0; j<slave->desc.fmt_count; ++j) {
		    ...
		    if (p == pt && pj_isdigit(*slave->desc.fmt[j].ptr)) {
				pjmedia_sdp_attr *a;
				pjmedia_sdp_rtpmap or_;
				/* dingcheng modify: pt = 9代表G.722编码，这种编码的clockrate应该是16000。
				 * 但事实上，由于某些历史原因，有部分终端会把G.722编码的clockrate配置为8000。
				 * 如果pjsip与这样的终端协商且优先选择了G.722编码，则会导致呼叫失败。
				 * 为了应对这种情况，我们在pjsip的sdp-match过程中补充对clockrate的判断。
				 */
				if (pt == 9)
				{
