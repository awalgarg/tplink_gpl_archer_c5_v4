...
// ycw-pjsip.Atheros
PJ_DEF(pj_status_t) pjmedia_sdp_attr_get_gpmd( const pjmedia_sdp_attr *attr,
					       pjmedia_sdp_gpmd *gpmd)
{
    const char *p = attr->value.ptr;
    const char *end = attr->value.ptr + attr->value.slen;
    pj_str_t token;

    PJ_ASSERT_RETURN(pj_strcmp2(&attr->name, "gpmd")==0, PJ_EINVALIDOP);

    /* gpmd BNF:
     *	a=gpmd:<payloadtype> <gpmd specific parameter>
     */
    ...
    /* Expecting space after payloadtype. */
    if (*p != ' ') return PJMEDIA_SDP_EINFMTP;

    /* Get space. */
    ++p;

    /* Set the remaining string as gpmd parameter. */
    gpmd->param.ptr = (char*)p;
    gpmd->param.slen = end - p;
					       ...
					       pjmedia_sdp_attr **p_attr)
{
    ...
    len = pj_ansi_snprintf(tempbuf, sizeof(tempbuf), 
			   "%.*s %.*s/%u%s%.*s",
			   (int)rtpmap->pt.slen,
			   pj_strnull(rtpmap->pt.ptr),
			   (int)rtpmap->enc_name.slen,
			   pj_strnull(rtpmap->enc_name.ptr),
			   rtpmap->clock_rate,
			   (rtpmap->param.slen ? "/" : ""),
			   (int)rtpmap->param.slen,
			   pj_strnull(rtpmap->param.ptr));
...
static int print_connection_info( pjmedia_sdp_conn *c, char *buf, int len)
{
    ...
    printed = pj_ansi_snprintf(buf, len, "c=%.*s %.*s %.*s\r\n",
			       (int)c->net_type.slen,
			       pj_strnull(c->net_type.ptr),
			       (int)c->addr_type.slen,
			       pj_strnull(c->addr_type.ptr),
			       (int)c->addr.slen,
			       pj_strnull(c->addr.ptr));
...
pjmedia_sdp_media* pjmedia_sdp_media_clone(
						 pj_pool_t *pool, 
...
pjmedia_sdp_attr* pjmedia_sdp_media_find_attr(
...
pjmedia_sdp_attr* pjmedia_sdp_media_find_attr2(
				const pjmedia_sdp_media *m,
 ...
 */
int pjmedia_sdp_print( const pjmedia_sdp_session *desc, 
