...
/* G711 factory private data for full codec manager */
...
/*G711 factory private data for lite codec manager*/
static struct g711_factory_lite
{
	pjmedia_codec_factory	base;
	pjmedia_endpt				*endpt;
	pj_pool_t					*pool;
...
pj_status_t pjmedia_codec_g711_init(pjmedia_endpt *endpt)
{
    ...
    if (g711_factory.endpt != NULL)
	 ...
	 #else
	 if (g711_factory.endpt != NULL && g711_factory_lite.endpt != NULL)
	 {
	 	/*Already initialized.*/
		return PJ_SUCCESS;
	 }
    ...
    /* Register codec factory to endpoint. */
   	status = pjmedia_codec_mgr_register_factory(full_codec_mgr, &g711_factory.base);
	 	if (status != PJ_SUCCESS)
	 	{
	 		return status;
	 	}
	...
	if (NULL == g711_factory_lite.endpt)
	{
		/* Init factory */
	   g711_factory_lite.base.op = &g711_factory_op;
	   g711_factory_lite.base.factory_data = NULL;
	   g711_factory_lite.endpt = endpt;

	   pj_list_init(&g711_factory_lite.codec_list);

	   /* Create pool */
	   g711_factory_lite.pool = pjmedia_endpt_create_pool(endpt, "g711_lite", 4000, 4000);
	   if (!g711_factory_lite.pool)
	   {
			return PJ_ENOMEM;
	   }

	   /* Create mutex. */
	   status = pj_mutex_create_simple(g711_factory_lite.pool, "g611_lite", &g711_factory_lite.mutex);
	   if (status != PJ_SUCCESS)
	   {
			goto on_error;
	   }
	   ...
	   /* Get the codec manager. */
		...
		status = pjmedia_codec_mgr_register_factory(lite_codec_mgr, &g711_factory_lite.base);
		if (status != PJ_SUCCESS)
		{
		 	return status;
		}		 
...
on_error:
    ...
    if (g711_factory.pool)
	 ...
	 if (g711_factory_lite.mutex)
	 {
	 	pj_mutex_destroy(g711_factory_lite.mutex);
		g711_factory_lite.mutex = NULL;
	 }

	 if (g711_factory_lite.pool)
	 {
	 	pj_pool_release(g711_factory_lite.pool);
		g711_factory_lite.pool = NULL;
	 }
...
pj_status_t pjmedia_codec_g711_deinit(void)
{
    ...
    if (g711_factory.endpt == NULL && g711_factory_lite.endpt == NULL)
	 ...
	 if (g711_factory.endpt)
	 {
    /* Lock mutex. */
    pj_mutex_lock(g711_factory.mutex);

    /* Get the codec manager. */
		full_codec_mgr = pjmedia_endpt_get_full_codec_mgr(g711_factory.endpt);
		if (!full_codec_mgr)
	 {
		...
		status = pjmedia_codec_mgr_unregister_factory(full_codec_mgr, &g711_factory.base);
    ...
    g711_factory.pool = NULL;
	 ...
	 if (g711_factory_lite.endpt)
	 {
	    /* Lock mutex. */
	    pj_mutex_lock(g711_factory_lite.mutex);

	    /* Get the codec manager. */
		 lite_codec_mgr = pjmedia_endpt_get_lite_codec_mgr(g711_factory_lite.endpt);
		 if (!lite_codec_mgr)
		 {
		 	g711_factory_lite.endpt = NULL;
			pj_mutex_unlock(g711_factory_lite.mutex);
			return PJ_EINVALIDOP;
		 }
		 status = pjmedia_codec_mgr_unregister_factory(lite_codec_mgr, &g711_factory.base);
