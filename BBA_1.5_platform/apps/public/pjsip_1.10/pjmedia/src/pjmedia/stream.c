...
#if 0 /*ycw-pjsip:I move this type define to stream.h*/
...
struct pjmedia_stream
{
   ...
   pj_str_t	port_info_encoding_name;	/**< Encoding name.		    */
	unsigned port_info_samples_per_frame;
	unsigned port_info_clock_rate;
	unsigned port_info_channel_count;
	#endif
				  ...
				  const void *payload, unsigned payloadlen)
{
	...
	PJ_LOG(5,(stream->port.info.name.ptr, 
		  "Ignored RTP pkt with bad DTMF event %d",
    		  event->event));
		#else
	PJ_LOG(5,(stream->port_info_name.ptr, 
		  "Ignored RTP pkt with bad DTMF event %d",
    		  event->event));	
		#endif
	return;
    ...
    PJ_LOG(5,(stream->port.info.name.ptr, "Received DTMF digit %c, vol=%d",
    	      ...
    	      (event->e_vol & 0x3F)));
	 #else
	 PJ_LOG(5,(stream->port_info_name.ptr, "Received DTMF digit %c, vol=%d",
    	      digitmap[event->event],
    	      (event->e_vol & 0x3F)));
	 #endif
 ...
 * in the RTP socket.
 * This function handles T38 Fax with UDPTL transport
 */
static void on_rx_rtp_t38( void *data, 
                       void *pkt,
...

{
    ...
    struct pjmedia_udptl_buf   *p_udptl_buf;
    ...
    if (bytes_read < 0) {
        ...
        PJ_LOG(1,(stream->port.info.name.ptr,"Receive 0 length packet T38, ignore\n"));
        return;
    } else if (bytes_read > MAX_UDPTL_BUF_SIZE) {
        PJ_LOG(1,(stream->port.info.name.ptr, "Drop UDPTL packet len %d greater than MAX_UDPTL_BUF_SIZE %d", 
                   bytes_read, MAX_UDPTL_BUF_SIZE));
        return;
    }

    pj_mutex_lock( stream->jb_mutex );
    
    if(stream->udptl_n == MAX_UDPTL) {
        PJ_LOG(1,(THIS_FILE, "Error: UDPTL packet buffer overflow, max number of packet is %d !\n", MAX_UDPTL));
    }
    
    p_udptl_buf = &stream->udptl_buf[stream->udptl_windex];
    p_udptl_buf->buf_len = bytes_read;
    pj_memcpy(p_udptl_buf->pbuf, pkt, bytes_read);

    /* for stats */
    stream->total_udptl_rx++;
    if (callStatP) {
    callStatP->RxPackets = stream->total_udptl_rx;
    callStatP->IncomingPacketSize = bytes_read;
    }    

    // PJ_LOG(3,(stream->port.info.name.ptr,"Total Rx udptl packets %d, len=%d", stream->total_udptl_rx, bytes_read));
    
    if(++stream->udptl_windex == MAX_UDPTL) {
        stream->udptl_windex = 0;
    }
    if(++stream->udptl_n > MAX_UDPTL) {
        stream->udptl_n = MAX_UDPTL;
    }
 ...
 * in the RTCP socket. 
 */
static void on_rx_rtcp_t38( void *data,
                        void *pkt, 
                        pj_ssize_t bytes_read)
{
    PJ_LOG(1,(THIS_FILE, "Error: should not reach here %s !!! \n", __FUNCTION__));
}
#	endif
...
#	else
static void on_rx_rtp( void *data, void *tp, pj_ssize_t bytes_read)
#	endif

{
    pjmedia_stream *stream = (pjmedia_stream*) data;
	 pjmedia_transport *transport = (pjmedia_transport*)tp;
	 char* pkt = NULL;
	 /* Added by sirrain zhang */
...
/*for dtmf receiver*/
	...
	if (bytes_read < 0)
	{
		LOGERR_((stream->port_info_name.ptr, "RTP recv() error", -bytes_read));
	}

	pkt = pjmedia_transport_udp_get_rtp_pkt(transport);
	
#ifdef INCLUDE_USB_VOICEMAIL	
	if (usbvm_matchRecordId(stream, &recordId) == TRUE)
	{
		/* Ignore keep-alive packets */
	    if (bytes_read < (pj_ssize_t) sizeof(pjmedia_rtp_hdr))
	    {
			LOGERR_((stream->port_info_name.ptr, "RTP recv() error, may be keep-alive pkt", -bytes_read));	
			return;
	    }

#	if (!(defined(PJ_MEDIA_TRANSIT_BY_PJSIP) && 0==PJ_MEDIA_TRANSIT_BY_PJSIP) || \
			defined(INCLUDE_PSTN_GATEWAY))
		 if (transport == stream->dsp_transport)
		 {
		 	return;
		 }
#	endif
		...
		/* Decode rtp packet . */
	    status = pjmedia_rtp_decode_rtp((pjmedia_rtp_session *)NULL, pkt, bytes_read,
					    &hdr, &payload, &payloadlen);
	    if (status != PJ_SUCCESS)
		 {
			LOGERR_((stream->port_info_name.ptr, "RTP decode error", status));
			return;
	    }
		...
		/* 因为语音信箱和普通通话是相互独立的，因此还是分别调用dtmf接受函数，sipinfo方式跟以前一样  */
/* Edited by huanglei, 2012/02/06 */
		usbvm_remoteRtpCopy(recordId, pkt, bytes_read);
/* end huanglei */
	}
	else
#endif /* INCLUDE_USB_VOICEMAIL */
	{	
		/*ycw-pjsip.20111104. My be the other transport has been destroyed,so, we
		must check it.*/		
		if ((transport==stream->transport && stream->transport!=NULL)
#	if (!(defined(PJ_MEDIA_TRANSIT_BY_PJSIP) && 0==PJ_MEDIA_TRANSIT_BY_PJSIP) || \
			defined(INCLUDE_PSTN_GATEWAY))
			&&(stream->dsp_transport!=NULL)
#	endif
	)
		{	
/*Edited by huanglei, 2011/11/07*/
			DR_parseDTMFRtpPacket(stream->callId, (pjmedia_rtp_hdr *)pkt, bytes_read, &dtmfNum, NULL, NULL);
/*end huanglei*/

#	if ((defined(PJ_MEDIA_TRANSIT_BY_PJSIP) && 0==PJ_MEDIA_TRANSIT_BY_PJSIP) && \
			defined(INCLUDE_PSTN_GATEWAY))
			if (PJMEDIA_TRANSPORT_VOIP2PSTN==stream->mediaTpType)
			{
#	endif

#	if !(defined(PJ_MEDIA_TRANSIT_BY_PJSIP) && 0==PJ_MEDIA_TRANSIT_BY_PJSIP) || \
			defined(INCLUDE_PSTN_GATEWAY)
				pjmedia_transport_send_rtp(stream->dsp_transport, pkt, bytes_read);
#	endif
...
#	if (!(defined(PJ_MEDIA_TRANSIT_BY_PJSIP) && 0==PJ_MEDIA_TRANSIT_BY_PJSIP) || \
			defined(INCLUDE_PSTN_GATEWAY))
		else if ((transport==stream->dsp_transport && stream->dsp_transport!=NULL)
...
#	if (defined(PJ_MEDIA_TRANSIT_BY_PJSIP) && 0==PJ_MEDIA_TRANSIT_BY_PJSIP) && \
			...
			{
#	endif
				pjmedia_transport_send_rtp(stream->transport, pkt, bytes_read);
#	if (defined(PJ_MEDIA_TRANSIT_BY_PJSIP) && 0==PJ_MEDIA_TRANSIT_BY_PJSIP) && \
			defined(INCLUDE_PSTN_GATEWAY)
			}
#	endif
...
#	endif
		...
		{
			PJ_LOG(4, (THIS_FILE, "%s,%d:Wrong transport!", __FUNCTION__, __LINE__));
			return;
		}
    ...
    if (seq_st.status.value)
	 {
		...
		TRC_  ((stream->port.info.name.ptr, 
		#else
		TRC_  ((stream->port_info_name.ptr, 
		#endif
	...
	if (seq_st.status.flag.badpt)
	{
	   ...
	   PJ_LOG(4,(stream->port.info.name.ptr,
		...
		#else
		PJ_LOG(4,(stream->port_info_name.ptr,
		      "Bad RTP pt %d (expecting %d)",
		      hdr->pt, channel->rtp.out_pt));
		#endif
	}
    ...
    if (seq_st.status.flag.restart)
	 {
		...
		#else
		PJ_LOG(4,(stream->port_info_name.ptr, "Jitter buffer reset"));
		#endif
	 ...
	 else
	 {
		...
		if (status != PJ_SUCCESS)
		{
		    ...
		    LOGERR_((stream->port.info.name.ptr, 
			     ...
			     status));
			#else
			LOGERR_((stream->port_info_name.ptr, 
			     "Codec parse() error", 
			     status));
			#endif
	...
	if (stream->has_g722_mpeg_bug) {
		    ...
		    stream->rtp_rx_last_cnt > 0)
		{
		    ...
		    frm_ts_span = stream->port.info.samples_per_frame /
				  ...
				  stream->port.info.channel_count;
			 #else
			 frm_ts_span = stream->port_info_samples_per_frame /
				  stream->codec_param.setting.frm_per_pkt/
				  stream->port_info_channel_count;
			 #endif
...
/*ycw-pjsip--delete Jitter Buffer*/
	...
	LOGERR_((stream->port.info.name.ptr, "Jitter buffer put() error", 
		status));
		#else
	LOGERR_((stream->port_info_name.ptr, "Jitter buffer put() error", 
		status));		
		#endif
	pkt_discarded = PJ_TRUE;
	goto on_return;
    ...
    if (bytes_read < 0)
	 {
		...
		LOGERR_((stream->port.info.name.ptr, "RTCP recv() error", 
		 -bytes_read));
		#else
		LOGERR_((stream->port_info_name.ptr, "RTCP recv() error", -bytes_read));	
		#endif
		return;
    }

	 pkt = pjmedia_transport_udp_get_rtcp_pkt(transport);
...
#	if !(defined(PJ_MEDIA_TRANSIT_BY_PJSIP) && 0==PJ_MEDIA_TRANSIT_BY_PJSIP) || \
			defined(INCLUDE_PSTN_GATEWAY)
	 		pjmedia_transport_send_rtcp(stream->dsp_transport, pkt, bytes_read);
#	endif
...
#	if (defined(PJ_MEDIA_TRANSIT_BY_PJSIP) && PJ_MEDIA_TRANSIT_BY_PJSIP==0) && \
		...
		{
#	endif
		 	pjmedia_transport_send_rtcp(stream->transport, pkt, bytes_read);
#	if (defined(PJ_MEDIA_TRANSIT_BY_PJSIP) && PJ_MEDIA_TRANSIT_BY_PJSIP==0) && \
			defined(INCLUDE_PSTN_GATEWAY)
		}
#	endif
				   ...
				   pjmedia_channel **p_channel)
{
	...
	/*ycw-pjsip. t38*/
	 if (channel->out_pkt_size < MAX_UDPTL_BUF_SIZE)
	 {
	 	channel->out_pkt_size = MAX_UDPTL_BUF_SIZE;
	 }
...
#	if (!(defined(PJ_MEDIA_TRANSIT_BY_PJSIP) && 0==PJ_MEDIA_TRANSIT_BY_PJSIP) || \
			defined(INCLUDE_PSTN_GATEWAY))
	 /*ycw-pjsip*/
	 pj_sockaddr remoteDspAddr; /*address of DSP RTP*/
	 pj_sockaddr remoteDspRtcpAddr; /*address of DSP RTCP*/
	 pj_str_t	loopbackIp = {"127.0.0.1", 9};
#	endif
    ...
    pjmedia_port_info_init(&stream->port.info, &name,
			   ...
			   16, 80);
	 #else
	 stream->port_info_name = name;
	 #if 0
	 stream->port_info_channel_count = info->fmt.channel_cnt;
	 #endif
	 #endif
...
#	if defined(PJ_MEDIA_TRANSIT_BY_PJSIP) && PJ_MEDIA_TRANSIT_BY_PJSIP==0
	 stream->mediaTpType = mediaTpType;
	CMSIP_PRINT("-----Media Transport Type is [%d]----", mediaTpType);
#	endif
	 #if 0
    ...
    *p++ = '.'; *p++ = 'o'; *p++ = 'r'; *p++ = 'g';
    ...
    if (stream->codec_param.info.max_bps < stream->codec_param.info.avg_bps)
    {
	 ...
	 /*ycw-pjsip. t38*/
	 stream->codec_param.setting.frm_per_pkt = info->ptime / info->param->info.frm_ptime;
	 /* Check for invalid frame per packet. */
    if (stream->codec_param.setting.frm_per_pkt < 1 ||
	 	stream->codec_param.info.fmt_id == PJMEDIA_FORMAT_T38)/*not for t38*/
    {
    ...
    if (PJMEDIA_STREAM_VAD_SUSPEND_MSEC > 0 && stream->vad_enabled)
	 {
		...
		#else
		PJ_LOG(4,(name.ptr,"VAD temporarily disabled"));
		#endif
    }
    /* Get the frame size */
    ...
    stream->rtp_rx_ts_len_per_frame = stream->port.info.samples_per_frame /
				      ...
				      stream->codec_param.info.channel_cnt;
	 #else
	 stream->rtp_rx_ts_len_per_frame = stream->port_info_samples_per_frame /
				      stream->codec_param.setting.frm_per_pkt /
				      stream->codec_param.info.channel_cnt;
	 #endif
...
#if 0
    if (stream->codec_param.info.fmt_id != PJMEDIA_FORMAT_T38) {
    ...
    else
    {
        ...
        /* Init T38 packet buffer*/
        int i;
        for(i=0; i<MAX_UDPTL; i++) {
            stream->udptl_buf[i].pbuf = pj_pool_zalloc(pool, MAX_UDPTL_BUF_SIZE);
        }
        stream->udptl_rindex = 0;
        stream->udptl_windex = 0;
        stream->udptl_n = 0;
        stream->total_udptl_rx = 0;
        stream->total_udptl_tx = 0;
    }
...

    {
		...
		#else
		rtcp_setting.samples_per_frame = stream->port_info_samples_per_frame;
		#endif
	...
	/*ycw-pjsip:attach the transport which communicate with DSP*/
	/*这儿的传输类型无需判断是UDP还是ICE的，因为它是内部通信，一定
	是UDP的，IP一定是环回接口的IP*/
#	if (!(defined(PJ_MEDIA_TRANSIT_BY_PJSIP) && 0==PJ_MEDIA_TRANSIT_BY_PJSIP) || \
			defined(INCLUDE_PSTN_GATEWAY))
	if (0 == g_dspPort || g_dspPort+2 > INT_MAX)
	{
		g_dspPort = DSP_RTP_PORT_START;
	}
	else
	{
		g_dspPort += 2;
	}
	pj_sockaddr_init(pj_AF_INET(), &remoteDspAddr, &loopbackIp, g_dspPort);
	pj_sockaddr_init(pj_AF_INET(), &remoteDspRtcpAddr, &loopbackIp, g_dspPort+1);
	status = pjmedia_transport_attach(dsp_tp, stream, &remoteDspAddr, &remoteDspRtcpAddr, 
		 				pj_sockaddr_get_len(&remoteDspAddr), &on_rx_rtp, &on_rx_rtcp);
	if (status != PJ_SUCCESS)
	{
		goto err_cleanup;
	}

	stream->dsp_transport = dsp_tp;
#	endif
...
/*delete Jitter Buffer*/
    ...
    *p_stream = stream;
	...
	/*ycw-pjsip--delete pjmedia_port*/
	PJ_LOG(5,(THIS_FILE, "Stream %s created", stream->port_info_name.ptr));
    ...
    if (stream->enc && stream->transport)
	 {
		...
		if (len != 0)
		{
	   	pjmedia_transport_send_rtcp(stream->transport, stream->enc->out_pkt, len);
		}
    }
#	endif
	...
	/*ycw-firewall*/
#	if defined(PJSIP_HAS_FIREWALL_FILTER)&& PJSIP_HAS_FIREWALL_FILTER!=0
	for (index = 0; index < PJ_ARRAY_SIZE(stream->fwRule); ++index)
	{
		pFwRule = &stream->fwRule[index];
		pj_firewall_set_rule_accept(PJ_FIREWALLCFG_DEL, pFwRule->protocol, 
			&pFwRule->destination, NULL, pFwRule->dport, 
			&pFwRule->source, NULL, pFwRule->sport, ctrl);
	}	
#	endif
...
#	if (!(defined(PJ_MEDIA_TRANSIT_BY_PJSIP) && 0==PJ_MEDIA_TRANSIT_BY_PJSIP) || \
			defined(INCLUDE_PSTN_GATEWAY))
	 /*ycw-pjsip*/
	 if (stream->dsp_transport)
	 {
	 	pjmedia_transport_detach(stream->dsp_transport, stream);
		stream->dsp_transport = NULL;
	 }
#	endif
...
pj_status_t pjmedia_stream_start(pjmedia_stream *stream)
{
    ...
    if (stream->enc && (stream->dir & PJMEDIA_DIR_ENCODING))
   {
		...
		#else
		PJ_LOG(4,(stream->port_info_name.ptr, "Encoder stream started"));
		#endif
    }
   else 
    {
		...
		#else
		PJ_LOG(4,(stream->port_info_name.ptr, "Encoder stream paused"));
		#endif
    }
    ...
    if (stream->dec && (stream->dir & PJMEDIA_DIR_DECODING))
   {
		...
		#else
		PJ_LOG(4,(stream->port_info_name.ptr, "Decoder stream started"));
		#endif
    }
   else 
   {
   	/*ycw-pjsip--delete pjmedia_port*/
		...
		#else
		PJ_LOG(4,(stream->port_info_name.ptr, "Decoder stream paused"));
		#endif
    }
#endif
    return PJ_SUCCESS;
    ...
    if ((dir & PJMEDIA_DIR_ENCODING) && stream->enc)
	 {
		...
		#else
		PJ_LOG(4,(stream->port_info_name.ptr, "Encoder stream resumed"));
		#endif
    }
    ...
    if ((dir & PJMEDIA_DIR_DECODING) && stream->dec)
	 {
		...
		#else
		PJ_LOG(4,(stream->port_info_name.ptr, "Decoder stream resumed"));
		#endif
    }
#endif
    return PJ_SUCCESS;
