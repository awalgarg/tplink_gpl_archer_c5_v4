...
#yuchuwei-modify commands of 'clean' targets
GCC-NOC := $(subst -c,,$(CC))
...
#Is it a static library or a dynamic library? by yuchuwei
ifeq ($(suffix $(LIB)), .a)
	echo "It is a static library"
	$(AR) $(LIB) $(OBJS)
	$(RANLIB) $(LIB)
endif
ifeq ($(suffix $(LIB)), .so)
	echo "It is a dynamic library"
	$(GCC-NOC) -shared -fPIC -o $(LIB) $(OBJS)	
endif

$(EXE): $(OBJDIRS) $(OBJS) $($(APP)_EXTRA_DEP)
	if test ! -d $(BINDIR); then $(subst @@,$(subst /,$(HOST_PSEP),$(BINDIR)),$(HOST_MKDIR)); fi
	$(LD) $($(APP)_CFLAGS)  $(LDOUT)$(subst /,$(HOST_PSEP),$(EXE)) \
...
#ycw-20120622
	...
	$(subst @@,$(subst /,$(HOST_PSEP),$(LIBDIR)/*),$(HOST_RMR))
	$(subst @@,$(subst /,$(HOST_PSEP),$(BINDIR)/*),$(HOST_RMR))
ifeq ($(OS_NAME),linux-kernel)
	rm -f ../lib/$(app).o
endif
