...
/* 
 * brief	Account Authenticate state
 */
typedef enum __ACC_AUTH_STATE
{
	ACC_AUTH_NONE,	/*Need not authentication*/
	ACC_AUTH_DOING,	/*processing authentication*/
	ACC_AUTH_DONE	/*has completed authentication*/
}ACC_AUTH_STATE;
#endif
...
#	if defined(SUPPORT_ACCOUNT_RTT) && SUPPORT_ACCOUNT_RTT!=0
	/*each account has only one regc, we count RTT in regc in order to get 
	the higher precise*/
	struct timeval			regStart;
	struct timeval			regFirstRps;
#	endif
	 
#ifdef INCLUDE_TFC_ES
	ACC_AUTH_STATE	authState; /*record the state of the authentication process*/
#endif
