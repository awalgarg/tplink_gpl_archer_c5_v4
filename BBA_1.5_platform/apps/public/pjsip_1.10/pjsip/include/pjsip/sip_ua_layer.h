...
typedef struct pjsip_ua_init_param
{
	...
	/* 
	 * brief	According to RFC3960, if pjsip receives forked response 18x, send UPDATE to mute
	 *			the dialog, except the first dialog. If pjsip receives forked response 200, send
	 *			UPDATE to restore the normal media transport.
	 */
