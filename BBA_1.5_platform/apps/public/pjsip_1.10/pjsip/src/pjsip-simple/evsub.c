...
#include <pj/pool.h>
#include <pj/string.h>
/*ycw-pjsip. Include cmsip assert */
...
struct pjsip_evsub
{
    ...
    pjsip_transaction	 *pending_sub;	/**< Pending UAC SUBSCRIBE tsx.	    */
	 ...
	 /*ycw-pjsip. 添加该订阅是否已经成功的标志，这样子在terminate的时候可用于识别*/
	 pj_bool_t	isSuccess;
...
{
	CMSIP_ASSERT(sub!=NULL);
    return sub->state;
}

/*ycw-pjsip*/
pj_bool_t pjsip_evsub_get_prevState(pjsip_evsub *sub)
{
	CMSIP_ASSERT(sub!=NULL);	
	return sub->isSuccess;
}
void pjsip_evsub_set_success(pjsip_evsub *sub)
{
	CMSIP_ASSERT(sub!=NULL);
	sub->isSuccess = PJ_TRUE;
}
