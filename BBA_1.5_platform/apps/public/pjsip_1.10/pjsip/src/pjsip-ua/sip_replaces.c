/* $Id: sip_replaces.c 3553 2011-05-05 06:14:19Z nanang $ */
/* 
						   ...
						   pjsip_tx_data **p_tdata)
{
    ...
    *p_dlg = NULL;
     ...
     * if not. 
     */
    if (pjsip_msg_find_hdr_by_name(rdata->msg_info.msg, &STR_REPLACES, rep_hdr->next))
	 {
     ...
     */
    dlg = pjsip_ua_find_dialog(&rep_hdr->call_id, &rep_hdr->to_tag, &rep_hdr->from_tag, PJ_TRUE);
    ...
    if (inv->state <= PJSIP_INV_STATE_EARLY && inv->role != PJSIP_ROLE_UAC)
	 ...
	 #else
	 if (inv->state <= PJSIP_INV_STATE_EARLY && inv->role != PJSIP_ROLE_UAS)
	 {
		code = PJSIP_SC_CALL_TSX_DOES_NOT_EXIST;
		warn_text = "Found early INVITE session but not initiated by this UA";
		goto on_return;
