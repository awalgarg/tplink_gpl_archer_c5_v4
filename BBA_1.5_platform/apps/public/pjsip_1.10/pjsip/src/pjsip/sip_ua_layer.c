...
retry_on_deadlock:
	...
	if (mod_ua.param.mute_dlg_forked)
	{
		pjsip_cseq_hdr *cseq_hdr = rdata->msg_info.cseq;
		...
		{
			pjsip_dialog* dlg = dlg_set->dlg_list.next;
			
			dlg = dlg->next;
			while (dlg != (pjsip_dialog*)&dlg_set->dlg_list)
			{
				mod_ua.param.mute_dlg_forked(dlg, rdata, PJ_TRUE);
				dlg->mute = PJ_TRUE;
				dlg = dlg->next;
			}			
		}
	...
	if (mod_ua.param.mute_dlg_forked)
	{
		...
		if (cseq_hdr->method.id == PJSIP_INVITE_METHOD &&
		    ...
		    dlg->mute)
		{
			mod_ua.param.mute_dlg_forked(dlg, rdata, PJ_FALSE);
			dlg->mute = PJ_FALSE;
		}
	}
#endif
