...
const pjsip_hdr_name_info_t pjsip_hdr_names[] = 
{
    ...
    { "P-Asserted-Identity", 19, NULL},	//PJSIP_H_P_ASSERTED_IDENTITY,
	{ "P-Prefered-Identity", 19, NULL}, //PJSIP_H_P_PREFERED_IDENTITY,
#endif
...
void pjsip_method_set( pjsip_method *m, pjsip_method_e me )
{
...
pjsip_msg* pjsip_msg_clone( pj_pool_t *pool, const pjsip_msg *src)
{
    pjsip_msg *dst;
    const pjsip_hdr *sh;
...
///////////////////////////////////////////////////////////////////////////////
void* pjsip_hdr_clone( pj_pool_t *pool, const void *hdr_ptr )
{
...
void* pjsip_hdr_shallow_clone( pj_pool_t *pool, const void *hdr_ptr )
{
    const pjsip_hdr *hdr = (const pjsip_hdr*) hdr_ptr;
    return (*hdr->vptr->shallow_clone)(pool, hdr_ptr);
}

int pjsip_hdr_print_on( void *hdr_ptr, char *buf, pj_size_t len)
{
...
const pj_str_t* pjsip_get_status_text(int code)
{
...
void pjsip_generic_string_hdr_init2(pjsip_generic_string_hdr *hdr,
					    pj_str_t *hname,
					    pj_str_t *hvalue)
{
 ...
 */
static int pjsip_p_asserted_identity_hdr_print( pjsip_p_asserted_identity_hdr *hdr, 
				   char *buf, pj_size_t size);
static pjsip_p_asserted_identity_hdr* pjsip_p_asserted_identity_hdr_clone( pj_pool_t *pool, 
					         const pjsip_p_asserted_identity_hdr *rhs);
static pjsip_p_asserted_identity_hdr* pjsip_p_asserted_identity_hdr_shallow_clone( pj_pool_t *pool,
							 const pjsip_p_asserted_identity_hdr *rhs);


static pjsip_hdr_vptr p_asserted_identity_hdr_vptr = 
{
    (pjsip_hdr_clone_fptr) &pjsip_p_asserted_identity_hdr_clone,
    (pjsip_hdr_clone_fptr) &pjsip_p_asserted_identity_hdr_shallow_clone,
    (pjsip_hdr_print_fptr) &pjsip_p_asserted_identity_hdr_print,
};

pjsip_p_asserted_identity_hdr* pjsip_p_asserted_identity_hdr_init( pj_pool_t *pool,
					      void *mem )
{
    pjsip_p_asserted_identity_hdr *hdr = (pjsip_p_asserted_identity_hdr*) mem;

    PJ_UNUSED_ARG(pool);

    pj_bzero(mem, sizeof(pjsip_p_asserted_identity_hdr));
    init_hdr(hdr, PJSIP_H_P_ASSERTED_IDENTITY, &p_asserted_identity_hdr_vptr);
    pj_list_init(&hdr->params);
    return hdr;
}

pjsip_p_asserted_identity_hdr* pjsip_p_asserted_identity_hdr_create( pj_pool_t *pool )
{
    void *mem = pj_pool_alloc(pool, sizeof(pjsip_p_asserted_identity_hdr));
    return pjsip_p_asserted_identity_hdr_init(pool, mem);
}

pjsip_p_asserted_identity_hdr* pjsip_p_asserted_identity_hdr_set( pjsip_p_asserted_identity_hdr *hdr )
{
    hdr->type = PJSIP_H_P_ASSERTED_IDENTITY;
    hdr->name.ptr = pjsip_hdr_names[PJSIP_H_P_ASSERTED_IDENTITY].name;
    hdr->name.slen = pjsip_hdr_names[PJSIP_H_P_ASSERTED_IDENTITY].name_len;
    hdr->sname.ptr = pjsip_hdr_names[PJSIP_H_P_ASSERTED_IDENTITY].sname;
    hdr->sname.slen = 1;
    return hdr;
}
				   ...
				   char *buf, pj_size_t size)
{
    ...
    *buf++ = ' ';
    ...
    printed = pjsip_param_print_on(&hdr->params, buf, endbuf-buf, 
				   &pc->pjsip_TOKEN_SPEC,
				   &pc->pjsip_TOKEN_SPEC, ';');
    if (printed < 0)
	return -1;
...
static pjsip_p_asserted_identity_hdr* pjsip_p_asserted_identity_hdr_clone( pj_pool_t *pool, 
					         const pjsip_p_asserted_identity_hdr *rhs)
{
    pjsip_p_asserted_identity_hdr *hdr = pjsip_p_asserted_identity_hdr_create(pool);

    hdr->type = rhs->type;
    hdr->name = rhs->name;
    hdr->sname = rhs->sname;
    hdr->uri = (pjsip_uri*) pjsip_uri_clone(pool, rhs->uri);
    pjsip_param_clone( pool, &hdr->params, &rhs->params);
...
static pjsip_p_asserted_identity_hdr* pjsip_p_asserted_identity_hdr_shallow_clone( pj_pool_t *pool,
							 const pjsip_p_asserted_identity_hdr *rhs)
{
    pjsip_p_asserted_identity_hdr *hdr = PJ_POOL_ALLOC_T(pool, pjsip_p_asserted_identity_hdr);
    pj_memcpy(hdr, rhs, sizeof(*hdr));
    pjsip_param_shallow_clone( pool, &hdr->params, &rhs->params);
    return hdr;
}
...
///////////////////////////////////////////////////////////////////////////////
/*
 * P-Prefered-Identity header.
 * By YuChuwei, For Telefonica
 */
static int pjsip_p_prefered_identity_hdr_print( pjsip_p_prefered_identity_hdr *hdr, 
				   char *buf, pj_size_t size);
static pjsip_p_prefered_identity_hdr* pjsip_p_prefered_identity_hdr_clone( pj_pool_t *pool, 
					         const pjsip_p_prefered_identity_hdr *rhs);
static pjsip_p_prefered_identity_hdr* pjsip_p_prefered_identity_hdr_shallow_clone( pj_pool_t *pool,
							 const pjsip_p_prefered_identity_hdr *rhs);


static pjsip_hdr_vptr p_prefered_identity_hdr_vptr = 
{
    (pjsip_hdr_clone_fptr) &pjsip_p_prefered_identity_hdr_clone,
    (pjsip_hdr_clone_fptr) &pjsip_p_prefered_identity_hdr_shallow_clone,
    (pjsip_hdr_print_fptr) &pjsip_p_prefered_identity_hdr_print,
};

pjsip_p_prefered_identity_hdr* pjsip_p_prefered_identity_hdr_init( pj_pool_t *pool,
					      void *mem )
{
    pjsip_p_prefered_identity_hdr *hdr = (pjsip_p_prefered_identity_hdr*) mem;

    PJ_UNUSED_ARG(pool);

    pj_bzero(mem, sizeof(pjsip_p_prefered_identity_hdr));
    init_hdr(hdr, PJSIP_H_P_PREFERED_IDENTITY, &p_prefered_identity_hdr_vptr);
    pj_list_init(&hdr->params);
    return hdr;
}

pjsip_p_prefered_identity_hdr* pjsip_p_prefered_identity_hdr_create( pj_pool_t *pool )
{
    void *mem = pj_pool_alloc(pool, sizeof(pjsip_p_prefered_identity_hdr));
    return pjsip_p_prefered_identity_hdr_init(pool, mem);
}

pjsip_p_prefered_identity_hdr* pjsip_p_prefered_identity_hdr_set( pjsip_p_prefered_identity_hdr *hdr )
{
    hdr->type = PJSIP_H_P_PREFERED_IDENTITY;
    hdr->name.ptr = pjsip_hdr_names[PJSIP_H_P_PREFERED_IDENTITY].name;
    hdr->name.slen = pjsip_hdr_names[PJSIP_H_P_PREFERED_IDENTITY].name_len;
    hdr->sname.ptr = pjsip_hdr_names[PJSIP_H_P_PREFERED_IDENTITY].sname;
    hdr->sname.slen = 1;
    return hdr;
}
...
static pjsip_p_prefered_identity_hdr* pjsip_p_prefered_identity_hdr_clone( pj_pool_t *pool, 
					         const pjsip_p_prefered_identity_hdr *rhs)
{
    pjsip_p_prefered_identity_hdr *hdr = pjsip_p_prefered_identity_hdr_create(pool);
...
static pjsip_p_prefered_identity_hdr* pjsip_p_prefered_identity_hdr_shallow_clone( pj_pool_t *pool,
							 const pjsip_p_prefered_identity_hdr *rhs)
{
    pjsip_p_prefered_identity_hdr *hdr = PJ_POOL_ALLOC_T(pool, pjsip_p_prefered_identity_hdr);
    pj_memcpy(hdr, rhs, sizeof(*hdr));
    pjsip_param_shallow_clone( pool, &hdr->params, &rhs->params);
    return hdr;
}
...
pjsip_rr_hdr* pjsip_routing_hdr_set_rr( pjsip_routing_hdr *hdr )
{
...
pjsip_route_hdr* pjsip_routing_hdr_set_route( pjsip_routing_hdr *hdr )
{
