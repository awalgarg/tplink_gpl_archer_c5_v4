					  ...
					  pjsip_dialog **p_dlg)
{
	...
	CMSIP_PRINT("target(%.*s)", tmp.slen, tmp.ptr);
	...
	CMSIP_PRINT("================begin to parse uri param=====================");
    ...
    if (PJSIP_URI_SCHEME_IS_SIP(dlg->target) || PJSIP_URI_SCHEME_IS_SIPS(dlg->target))
    {
		...
		while (param != &uri->header_param)
		{
	   	...
	   	int c;
			...
			CMSIP_PRINT("befor parse:hdr(%.*s, %.*s)", param->name.slen, param->name.ptr,
    ...
    /* Init local contact. */
    pj_strdup_with_null(dlg->pool, &tmp, local_contact ? local_contact : local_uri);
    dlg->local.contact = (pjsip_contact_hdr*)pjsip_parse_hdr(dlg->pool, &HCONTACT, tmp.ptr, 
					 tmp.slen, NULL);
    ...
    if (msg->line.req.method.id != PJSIP_ACK_METHOD)
	 {
		...
		if (status != PJ_SUCCESS)
	   {
		...
		/*ycw-20120612*/
		/*If this is a subsequent request in a dialog, and the target of initial INVITE is a 
		domain name, pass the related informaiton of this domain name to the tsx*/
		if (dlg->server_name.slen)
		{
			pj_strdup(tsx->pool, &tsx->server_name, &dlg->server_name);
			memcpy(&tsx->server_addr, &dlg->server_addr, sizeof(dlg->server_addr) );
			tsx->cur_addr = dlg->cur_addr;
		}
		...
		/*ycw-20120612*/
		/*If target is a domain name, I save its related ip to use for subsequent request*/
		if (PJSIP_INVITE_METHOD == msg->line.req.method.id && 0 == dlg->server_name.slen)
		{
			pj_in_addr dummy;
			pj_in6_addr dummy6;

			if (	tdata->dest_info.name.slen
				&& 0 == pj_inet_aton(&tdata->dest_info.name, &dummy)
				&& pj_inet_pton(pj_AF_INET6(), &tdata->dest_info.name, &dummy6) != PJ_SUCCESS)			
			{
				CMSIP_PRINT("Domain Name================");
				pj_strdup(dlg->pool, &dlg->server_name, &tdata->dest_info.name);
				memcpy(&dlg->server_addr, &tdata->dest_info.addr, sizeof(dlg->server_addr) );
				dlg->cur_addr = tdata->dest_info.cur_addr;
			}
		}
    }
	 else
	 {
		...
		if (dlg->server_name.slen)
		{
			pj_strdup( tdata->pool, &tdata->dummy_dest_info.name, &dlg->server_name);
			memcpy(&tdata->dummy_dest_info.addr, &dlg->server_addr, sizeof(dlg->server_addr));
			tdata->dummy_dest_info.cur_addr = dlg->cur_addr;
		}

		/* Send request */
		status = pjsip_endpt_send_request_stateless(dlg->endpt, tdata, NULL, NULL);
		if (status != PJ_SUCCESS)
		{
			goto on_error;
		}
...
#ifdef INCLUDE_TFC_ES
	/*By YuChuwei, For Telefonica*/
	hdr = (pjsip_hdr*)pjsip_msg_find_hdr(tdata->msg, PJSIP_H_RECORD_ROUTE, pos);
	while (hdr)
	{
		pos = hdr->next;
		pj_list_erase(hdr);
...
#ifdef INCLUDE_TFC_ES
	/*Remove existing Record-Route header
	*By YuChuwei, For Telefonica
	*/
	hdr = (pjsip_hdr*) pjsip_msg_find_hdr(tdata->msg, PJSIP_H_RECORD_ROUTE, pos);
	while (hdr)
