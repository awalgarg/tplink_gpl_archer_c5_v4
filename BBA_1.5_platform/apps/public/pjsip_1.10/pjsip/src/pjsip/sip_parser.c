...
static pjsip_hdr* parse_hdr_p_prefered_identity ( pjsip_parse_ctx *ctx);
#endif
...
#ifdef INCLUDE_TFC_ES
	/*By YuChuwei, For Telefonica*/
	status = pjsip_register_hdr_parser( "P-Asserted-Identity", NULL, 
				&parse_hdr_p_asserted_identity);
    PJ_ASSERT_RETURN(status == PJ_SUCCESS, status);

	status = pjsip_register_hdr_parser( "P-Prefered-Identity", NULL,
				&parse_hdr_p_prefered_identity);
	PJ_ASSERT_RETURN(status == PJ_SUCCESS, status);
#endif
...
pj_bool_t pjsip_find_msg( const char *buf, pj_size_t size, 
					      ...
					      unsigned opt)
{
	pjsip_uri *uri;
	int is_name_addr = 0;
	/*By YuChuwei, For Telefonica, and others*/
	/*Support Tel Uri*/
	const pj_str_t STR_TEL_SCHEME = {"tel", 3};
	...
	else
	{
		...
		/*by YuChuwei, For Telefonica, and others*/
		/*Support Tel Uri*/
		if (pj_strcmp(&STR_TEL_SCHEME, &scheme) == 0)
		{
			opt &= ~PJSIP_PARSE_URI_AS_NAMEADDR;
		}
...
static pjsip_hdr* parse_hdr_p_asserted_identity ( pjsip_parse_ctx *ctx)
{
	pjsip_p_asserted_identity_hdr *hdr = pjsip_p_asserted_identity_hdr_create(ctx->pool);

    hdr->uri = int_parse_uri_or_name_addr(ctx->scanner, ctx->pool, 
					  PJSIP_PARSE_URI_AS_NAMEADDR |
					  PJSIP_PARSE_URI_IN_FROM_TO_HDR);
    ...
    while ( *ctx->scanner->curptr == ';' )
	{
		...
		int_parse_param( ctx->scanner, ctx->pool, &pname, &pvalue, 0);

	    pjsip_param *p = PJ_POOL_ALLOC_T(ctx->pool, pjsip_param);
	    p->name = pname;
	    p->value = pvalue;

	    pj_list_insert_before(&hdr->params, p);
    }

    parse_hdr_end(ctx->scanner);
	
    if (ctx->rdata)
        ctx->rdata->msg_info.assertedId = hdr;

    return (pjsip_hdr*)hdr;
}

/*By  YuChuwei, For Telefonica*/
/* Parse P-PREFERED-Identity: header*/
static pjsip_hdr* parse_hdr_p_prefered_identity ( pjsip_parse_ctx *ctx)
{
	pjsip_p_prefered_identity_hdr *hdr = pjsip_p_prefered_identity_hdr_create(ctx->pool);
    ...
    if (ctx->rdata)
        ctx->rdata->msg_info.preferedId = hdr;
...
static pjsip_hdr* parse_hdr_generic_string( pjsip_parse_ctx *ctx )
{
	...
	CMSIP_PRINT("after parse:hdr(%.*s, %.*s)", hdr->name.slen, hdr->name.ptr,
		hdr->hvalue.slen, hdr->hvalue.ptr);
    return (pjsip_hdr*)hdr;
