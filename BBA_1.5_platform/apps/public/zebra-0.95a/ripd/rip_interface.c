  ...
  for (node = listhead (iflist); node; nextnode (node))
      ...
      for (cnode = listhead (ifp->connected); cnode; nextnode (cnode))
	{
	  ...
	  if (if_is_pointopoint (ifp))
	    {
	      p = (struct prefix_ipv4 *) connected->address;
	      //TP -- src and destination are the same, it's ipoa, 
...
/* TP - bug fix, split_horizon is default to 1 to begin with, after reset, it should 
         remain 1; not 0. */
      ri->split_horizon = 1;
      ri->split_horizon_default = 1;
...
#if defined(TP_AUTH_SUPPORT) && defined(TP_CMD_SUPPORT)
...
#if defined(TP_CMD_SUPPORT) && defined(TP_AUTH_SUPPORT)
