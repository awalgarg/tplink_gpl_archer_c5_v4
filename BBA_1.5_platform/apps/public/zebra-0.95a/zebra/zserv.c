...
#ifdef TP_CMM_MSG
static int
tp_zclient_socket_un (char *path)
{
  int ret;
  int sock, len;
...
void zsend_cos_route_info(CMSG_BUFF *pMsgBuff)
{
  int cos_fd = -1;
  unsigned char cosPath[16];

  printf("send msg to cos\n");
  
  sprintf(cosPath, "/var/tmp/%d", CMSG_ID_COS);
  cos_fd = tp_zclient_socket_un(cosPath);
  if (cos_fd < 0)
	fprintf(stderr, "fail to send route info\n");

  writen(cos_fd, pMsgBuff, sizeof(CMSG_BUFF));
  close(cos_fd);	
}
#endif
...
#ifdef TP_CMM_MSG
  CMSG_BUFF msgBuff;
  ZEBRA_ROUTE_INFO_MSG *pRouteMsg = (ZEBRA_ROUTE_INFO_MSG *)(msgBuff.content);

  memset(&msgBuff, 0, sizeof (CMSG_BUFF));
  msgBuff.type = CMSG_ZEBRA_ROUTE;
  pRouteMsg->isAdd = 1;
#endif
...
#ifdef TP_CMM_MSG
		  pRouteMsg->nexthop[pRouteMsg->nexthop_num].v4_addr = nexthop.s_addr;
		  pRouteMsg->nexthop_num++;
#endif
	      break;
	    case ZEBRA_NEXTHOP_IPV6:
...
#ifdef TP_CMM_MSG
  pRouteMsg->family = p.family;
  pRouteMsg->type = rib->type;
  pRouteMsg->prefixlen = p. prefixlen;
  pRouteMsg->prefix4 = p.prefix.s_addr;
  pRouteMsg->distance = rib->distance;
  pRouteMsg->metric = rib->metric;
  zsend_cos_route_info(&msgBuff);
#endif
}
...
#ifdef TP_CMM_MSG
  pRouteMsg->family = p.family;
  pRouteMsg->type = api.type;
  pRouteMsg->prefixlen = p. prefixlen;
  pRouteMsg->prefix4 = p.prefix.s_addr;
  pRouteMsg->ifIndex = ifindex;
  pRouteMsg->nexthop[0].v4_addr = nexthop.s_addr;
  pRouteMsg->nexthop_num++;
  pRouteMsg->distance = api.distance;
  pRouteMsg->metric = api.metric;
  zsend_cos_route_info(&msgBuff);
#endif
}
...
zread_ipv6_add (struct zserv *client, u_short length)
{
  ...
  pRouteMsg->type = api.type;
  pRouteMsg->prefixlen = p. prefixlen;
  memcpy(pRouteMsg->prefix6, &p.prefix, p.prefixlen);
  pRouteMsg->ifIndex = ifindex;

  if (!IN6_IS_ADDR_UNSPECIFIED (&nexthop))
  {
  	pRouteMsg->nexthop_num++;
	memcpy(pRouteMsg->nexthop[0].v6_addr, &nexthop, sizeof(nexthop));
  }
  pRouteMsg->distance = api.distance;
  pRouteMsg->metric = api.metric;
  zsend_cos_route_info(&msgBuff);
...
#endif
  printf(stderr, "[zserv] receive COMMAND: %d\n", command);
  switch (command) 
    {
