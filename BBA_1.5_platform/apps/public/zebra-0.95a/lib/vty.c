...
enum event 
  {
    ...
    VTY_WRITE,
    VTY_TIMEOUT_RESET,
#if defined(VTYSH) || defined(TP_CMM_MSG)
    VTYSH_SERV,
    VTYSH_READ
#endif /* VTYSH */
  };
...
#if 0
{
	int vs = 0;
	printf("cmd vector:\n");
	while (vs < vector_max(vline))
	{
		printf("[%d] %s\n", vs, vector_slot(vline, vs));
		vs++;
	}
}
#endif
  ...
  {
  	vty->node = old_node == CONFIG_NODE ? RIPNG_NODE : CONFIG_NODE;
	ret = cmd_execute_command_strict(vline, vty, NULL);
  }
  ...
  if (ret != CMD_SUCCESS)
    switch (ret)
      {
      case CMD_WARNING:
	  fprintf (stderr, "Warning...%s", VTY_NEWLINE);
	break;
      case CMD_ERR_AMBIGUOUS:
	fprintf (stderr, "%% Ambiguous command.%s", VTY_NEWLINE);
	break;
      case CMD_ERR_NO_MATCH:
	fprintf (stderr, "%% Unknown command.%s", VTY_NEWLINE);
	break;
      case CMD_ERR_INCOMPLETE:
	fprintf (stderr, "%% Command incomplete.%s", VTY_NEWLINE);
	break;
      }
...
vty_execute (struct vty *vty)
{
  ...
  while(*cmd != '\0')
  {
	...
	if (next_cmd != NULL)*next_cmd = '\0';
	 /* the function only excute one command
	  * err happens while more than one.
	  */
...
#endif
#if defined(TP_CMD_SUPPORT) && !defined(TP_CMM_MSG)
...
#if defined(VTYSH) || defined(TP_CMM_MSG)
    case VTYSH_SERV:
	  printf("start VTYSH_SERV\n");
      thread_add_read (master, vtysh_accept, vty, sock);
      break;
    case VTYSH_READ:
      printf("start VTYSH_READ\n");
