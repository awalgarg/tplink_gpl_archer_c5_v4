...
/* Edited by Huang Lei, 2012/09/24 */
	int syncFlag =0;
/* Edited End */
	outsize = set_message(outbuf,0,0,True);
	...
	if(fsp->is_directory) {
		...
		/* Edited by Huang Lei, sync to disk right after closing file. 2012/09/24 */
		if(fsp->modified)
		{	
			syncFlag = 1;
		}
...
static void rename_open_files(connection_struct *conn, SMB_DEV_T dev, SMB_INO_T inode, char *newname)
{
	...
	/* Added by huang lei, to sync to disk when changed file/dir's name */
		sync_disk();
