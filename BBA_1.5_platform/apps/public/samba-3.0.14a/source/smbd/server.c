...
BOOL reload_services(BOOL test)
{
	...
	/* Modified by zjj, 20131101, for winxp, when we reload the service, pc client may not send
	 * a tdis(tree-disconnect) packet to the server, thus the connection is not closed, and 
	 * the function conn_snum_used will return true. We ignore it here to unload the services for
	 * the bug that windows client's share volume is still visible when we umount a volume and 
	 * reload the config.
	 */
...
//void build_options(BOOL screen); // modified by gengxianhu for removing debug information
 ...
 int main(int argc,const char *argv[])
{
	...
	while((opt = poptGetNextOpt(pc)) != -1) {
		switch (opt)  {
		case 'b':
			// modified by gengxianhu for removing debug information
