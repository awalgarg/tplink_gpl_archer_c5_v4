...
static BOOL reply_spnego_ntlmssp_android(connection_struct *conn, char *inbuf, char *outbuf,
				 AUTH_NTLMSSP_STATE **auth_ntlmssp_state,
				 DATA_BLOB *ntlmssp_blob, NTSTATUS nt_status) 
{
	BOOL ret;
	...
	if (NT_STATUS_IS_OK(nt_status)) {
		...
		response = data_blob(ntlmssp_blob->data, ntlmssp_blob->length);
        //response = spnego_gen_auth_response(ntlmssp_blob, nt_status, OID_NTLMSSP);
	ret = reply_sesssetup_blob(conn, outbuf, response, nt_status);
	data_blob_free(&response);
...
/****************************************************************************
 Reply to a android session setup spnego negotiate packet.
****************************************************************************/

static int reply_spnego_negotiate_android(connection_struct *conn, 
				  char *inbuf,
				  char *outbuf,
				  int length, int bufsize,
				  DATA_BLOB blob1)
{
	...
	secblob = data_blob(blob1.data, blob1.length);
	/* parse out the OIDs and the first sec blob */
//	if (!parse_negTokenTarg(blob1, OIDs, &secblob)) {
//		return ERROR_NT(NT_STATUS_LOGON_FAILURE);
//	}
...
//		free(OIDs[i]);
//	}
	DEBUG(1,("Got secblob of size %lu\n", (unsigned long)secblob.length));
...
//#endif
	...
	reply_spnego_ntlmssp_android(conn, inbuf, outbuf, &global_ntlmssp_state,
			     &chal, nt_status);
...
static int reply_spnego_auth_android(connection_struct *conn, char *inbuf, char *outbuf,
			     int length, int bufsize,
			     DATA_BLOB blob1)
{
	DATA_BLOB auth, auth_reply;
	...
	auth = data_blob(blob1.data, blob1.length);
	//if (!spnego_parse_auth(blob1, &auth)) {
#if 0
		file_save("auth.dat", blob1.data, blob1.length);
#endif
					...
					int length,int bufsize)
{
	...
	/*modify for android smb client, added by Zhu Haiming, 23May13.*/

	/*The package of some android phone client sended is not standard,
	it lose some field in Security Blob, and the Security Blob start with mechToken field.*/
	if (blob1.data[0] == ANS1_MECHTOKEN) {
		
		DEBUG(1, ("go to android reply, type:%d\n", SVAL(blob1.data, 8)));
		if(SVAL(blob1.data, 8) == 1)
		{
			/* its a negTokenTarg packer, but lose some field*/
			ret = reply_spnego_negotiate_android(conn, inbuf, outbuf, length, bufsize, blob1);
		}
		else
		{
			/* its a auth packet, but lose some field */
			ret = reply_spnego_auth_android(conn, inbuf, outbuf, length, bufsize, blob1);
		}
		data_blob_free(&blob1);
		return ret;
	}

	/*what sort of packed is this? send information to system log.*/
	if (1)
	{
		fstring debug = {0};
		int i, num;
		num = (blob1.length * 2 < FSTRING_LEN) ? blob1.length : (FSTRING_LEN / 2);
		for (i = 0; i < num; i++)
		{
			debug[2 * i] = ((blob1.data[i] >> 4) & 0x0F) + (((blob1.data[i] >> 4) & 0x0F) < 10 ? '0' : 'a' - 10);
			debug[2 * i + 1] = (blob1.data[i] & 0x0F) + ((blob1.data[i] & 0x0F) < 10 ? '0' : 'a' - 10);
		}
		debug[FSTRING_LEN - 1] = '\0';
		/*cmmlog(LOG_INFORM, LOG_OTHER, "SMB Unknown: %s\n", debug);*/
	}
	/*ended by Zhu Haiming, 23May13.*/
