...
/* 
 * brief	10 MB. Add by zengdongbiao.
 */
#define MAX_UNCOMPRESSDATA_SIZE	(10485760)
                            ...
                            AVDictionary **options)
{
    ...
    AVSubtitle subtitle;
    AVPacket pkt = *avpkt;
	/* used for counting frame->data size, Add by zengdongbiao */
	int idx = 0;	
	int dataSize = 0;
	...
	/* 统计解码后的数据大小，超过10M后停止解码, add by Zengdongbiao*/
	for (idx = 0; idx < AV_NUM_DATA_POINTERS; idx++)
	{
		if (frame->data[idx] != NULL)
		{
			dataSize += strlen(frame->data[idx]);
		}
	}
	if (dataSize > MAX_UNCOMPRESSDATA_SIZE)
	{
		av_log(s, AV_LOG_WARNING, "Using too much memory(%d bytes) during decoding, quitting decoding.\n", dataSize);
		ret = -2;
	}
	/* End add */
...
int avformat_find_stream_info(AVFormatContext *ic, AVDictionary **options)
{
	...
	int	decodeResult = 0;	/* Add by zengdongbiao  */
...
/*            if (!st->time_base.num)
    ...
    for (;;) {
         ...
         * the container. */
		decodeResult = try_decode_frame(ic, st, pkt,
		...
		/* 解码后一帧数据过大，为了避免使用过量内存，提前结束，zengdongbiao  */
		if (decodeResult == -2)
		{
			break;
		}
