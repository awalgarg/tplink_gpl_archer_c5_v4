...
image_new_from_jpeg(const char *path, int is_file, const uint8_t *buf, int size, int scale, int rotate)
{
	...
	/* The decompress scale = cinfo.scale_num / cinfo.scale_denom. Add by zengdongbiao, 19Mar15.*/
	cinfo.scale_num  = 1;
...
int image_getJpegSize(const char *path, int is_file, const uint8_t *buf, int size, int *width, int *height)
{
	...
	if (cinfo.image_width != 0 && cinfo.image_height != 0)
	{
		...
		*height = cinfo.image_height;
		//DPRINTF(E_WARN, L_METADATA, "image_getJpegSize: width=%d, height=%d\n", *width, *height);	
	}
	else /* backup, it seems never goto here.  */
	{
		DPRINTF(E_WARN, L_METADATA, "cant get image size from header, start decompress\n");
		/* The decompress scale = cinfo.scale_num / cinfo.scale_denom */
		cinfo.scale_num  = 1;
		cinfo.scale_denom = 1;
		cinfo.do_fancy_upsampling = FALSE;
		cinfo.do_block_smoothing = FALSE;
