...
insert_playlist(const char * path, char * name)
{
	...
	//DPRINTF(E_WARN, L_SCANNER, "Playlist %s contains %d items\n", name, items);
...
#ifdef LITE_MINIDLNA
		if( sql_get_int_field(db, "SELECT o.ID from OBJECTS o left join DETAILS d on "
								"(o.DETAIL_ID = d.ID) where o.PARENT_ID = '"MUSIC_PLIST_ID"'"
		                          " and d.TITLE = '%q'", plname) <= 0 )
		{
			detailID = GetFolderMetadata(plname, NULL, NULL, NULL, 0);
			sql_exec(db, "INSERT into OBJECTS"
			             " (OBJECT_ID, PARENT_ID, DETAIL_ID, CLASS) "
			             "VALUES"
			             " ('%s$%llX', '%s', %lld, 'container.%s')",
			             MUSIC_PLIST_ID, plID, MUSIC_PLIST_ID, detailID, class);
		}
#else
...
#else
				sql_exec(db, "INSERT into OBJECTS"
				             " (OBJECT_ID, PARENT_ID, CLASS, DETAIL_ID, REF_ID) "
				             "SELECT"
				             " '%s$%llX$%d', '%s$%llX', CLASS, DETAIL_ID, OBJECT_ID from OBJECTS"
				             " where DETAIL_ID = %lld and OBJECT_ID glob '" BROWSEDIR_ID "$*'",
				             MUSIC_PLIST_ID, plID, plist.track,
				             MUSIC_PLIST_ID, plID,
				             detailID);
