...
#else
	/* if use lite_minidlna, we only store directory path and title. 
	 * Add by zengdongbiao, 18May15.  
	 */
	ret = sql_exec(db, "INSERT into DETAILS"
	                   " (TITLE, PATH) "
	                   "VALUES"
...
GetAudioMetadata(const char *path, char *name)
{
	...
	//DPRINTF(E_WARN, L_METADATA, "Parsing audio %s...\n", path);

	if ( stat(path, &file) != 0 )
		return 0;
	
	/* to exclude empty file, add by zengdongbiao, 15Dec2014 */
	if (file.st_size <= 0)
	{
		DPRINTF(E_WARN, L_GENERAL, "empty file: %s\n", path);
		return 0;
	}
	...
	{
		strcpy(type, "ape");
		m.mime = strdup("audio/x-monkeys-audio");
	}
	else
	{
...
#else
	/* if use lite_minidlna, we only store file path, size, timestamp, title and mime. 
	 * Add by zengdongbiao, 18May15.  
	 */
	ret = sql_exec(db, "INSERT into DETAILS"
	                   " (PATH, SIZE, TIMESTAMP, TITLE, MIME) "
	                   "VALUES"
	                   " (%Q, %lld, %lld, %Q, %Q);",
...
GetImageMetadata(const char *path, char *name)
{
	...
	//DPRINTF(E_WARN, L_METADATA, "Parsing image %s...\n", path);
	if ( stat(path, &file) != 0 )
		return 0;
...
#ifndef LITE_MINIDLNA		
	/* if image size > 10M, don't handle it, add by liweijie,2014-08-01 */
	if (file.st_size > (10 * (1 << 20))) /* 10 * 1024 * 1024 */
	{
		return 0;
	}
#endif
...
#ifndef LITE_MINIDLNA
	/* soho don't handle exif data, we handle for certification, add by liwejie,2014-08-01 */
	// goto no_exifdata;
	...
	if( ed->size )
	{
		...
		if( ed->size > 12000 )
		{			
			/* Get image size without decompress, modify by zengdongbiao, 25May15. */
		#ifdef MEMORY_OPTIMIZE_FOR_ALBUMART
			if (image_getJpegSize(NULL, 0, ed->data, ed->size, &width, &height) == 0)
			{
...
no_exifdata:
    ...
    case IMAGE_TYPE_GIF:
    {
        ...
        if( ret != 0 )
        {
			...
			/* if avformat_open_input() successful, then close, add by zengdongbiao, 25Dec2014.  */
			if (ctx)
			{
				lav_close(ctx);
			}
		...
		/* For a gif picture,  iformat->name is image2 under ffmpeg-0.11.2, but is gif under 
		 * ffmpeg-2.5.2, modify by zengdongbiao, 20Jan15 */
        if (strcmp(ctx->iformat->name, "image2") != 0
			&& strcmp(ctx->iformat->name, "gif") != 0 )
        {
            DPRINTF(E_DEBUG, L_METADATA, "%s\n", path);
            lav_close(ctx);
            goto out;
	...
	//DPRINTF(E_DEBUG, L_METADATA, " * resolution: %dx%d\n", width, height);
...
out:
	...
	DPRINTF(E_DEBUG, L_METADATA, "dlna_pn: %s\n", m.dlna_pn);
...
GetVideoMetadata(const char *path, char *name)
{
	...
	//DPRINTF(E_WARN, L_METADATA, "Parsing video %s...\n", name);
	if ( stat(path, &file) != 0 )
		return 0;
...
video_no_dlna:
	...
	ret = sql_exec(db, "INSERT into DETAILS"
 	...
 	if (ends_with(path, ".mpg") || ends_with(path, ".mpeg") || ends_with(path, ".mkv") || ends_with(path, ".dat"))
    {
        m.mime = strdup("video/mpeg");
    }
	else if ( ends_with(path, ".avi") || ends_with(path, ".divx") || ends_with(path, ".xvid") )
    {
        m.mime = strdup("video/x-msvideo");
    }
	else if (ends_with(path, ".asf"))
    {
        m.mime = strdup("video/x-ms-asf");
    }
	else if (ends_with(path, ".wmv"))
    {
        m.mime = strdup("video/x-ms-wmv");
    }
	else if ( ends_with(path, ".mp4") || ends_with(path, ".m4v") || ends_with(path, ".3gp") )
    {
        m.mime = strdup("video/mp4");
    }
	else if ( ends_with(path, ".mts") || ends_with(path, ".m2ts") || ends_with(path, ".m2t") )
    {
        m.mime = strdup("video/vnd.dlna.mpeg-tts");
    }
	else if ( ends_with(path, ".vob"))
    {
        m.mime = strdup("video/mp2p");
    }
	else if ( ends_with(path, ".ts"))
    {
        m.mime = strdup("video/mpeg2");
    }
	...
	#ifdef TIVO_SUPPORT
	else if ( ends_with(path, ".TiVo"))
    {
        m.mime = strdup("video/x-tivo-mpeg");
    }
	#endif
	...
	else if ( ends_with(path, ".rmvb") || ends_with(path,".rm") )
	{
		m.mime = strdup("video/x-rmvb");
	}
    else 
    {
        DPRINTF(E_WARN, L_GENERAL, "Unhandled video extension on %s\n", path);
        return 0;
    }
...
#if (!defined LITE_MINIDLNA) && (defined USE_ALBUM_RESPONSE)
static uint8_t *
parseAlbumArtAudio(const char *path, int width, int height, int *pBufSize)
{
	char type[4];
	struct stat file;
	uint8_t *album_buf = NULL;
	struct song_metadata song;

	//DPRINTF(E_WARN, L_METADATA, "parseAlbumArtAudio %s...\n", path);
	...
	{
		DPRINTF(E_ERROR, L_METADATA, "Unhandled file extension on %s\n", path);
		return NULL;
	}

	if( readtags((char *)path, &song, &file, "en_US", type) != 0 )
	{
		DPRINTF(E_WARN, L_METADATA, "Cannot extract tags from %s!\n", path);
        	freetags(&song);
		return NULL;
	...
	*pBufSize = song.image_size;

	album_buf = get_embedded_art(song.image, width, height, pBufSize);
...
static uint8_t *
parseAlbumArtVideo(const char *path, int width, int height, int *pBufSize)
{
	struct stat file;
	int ret, i;
	AVFormatContext *ctx = NULL;
	...
	//DPRINTF(E_WARN, L_METADATA, "parseAlbumArtVideo %s...\n", path);
	...
	for( i=0; i<ctx->nb_streams; i++)
	{
		if(lav_is_thumbnail_stream(ctx->streams[i], &thumb_data, pBufSize))
		{
			break;
		}
	}

	if( (!thumb_data) && (strcmp(ctx->iformat->name, "asf") == 0) )
	{
		if( readtags((char *)path, &video, &file, "en_US", "asf") == 0 )
		{
			thumb_data = video.image;
			*pBufSize = video.image_size;
		}
	}

	if (thumb_data)
	{
		album_buf = get_embedded_art(thumb_data, width, height, pBufSize);
	}
...
uint8_t *
parseAlbumArt(const char *path, int width, int height, int *pBufSize, int *pAlbumType)
{
	uint8_t *pAlbumBuf = NULL;
	...
	*pAlbumType = EMBEDDED_ART;

	if (is_audio(path))
	{
		pAlbumBuf = parseAlbumArtAudio(path, width, height, pBufSize);
	}
	else if (is_video(path))
	{
		pAlbumBuf = parseAlbumArtVideo(path, width, height, pBufSize);
	}
	else
	{
		pAlbumBuf = get_album_file(path, width, height, pBufSize, pAlbumType);
	}

	return pAlbumBuf;
}
#endif /* (!defined LITE_MINIDLNA) && (defined USE_ALBUM_RESPONSE) */
