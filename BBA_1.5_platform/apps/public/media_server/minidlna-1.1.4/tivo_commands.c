...
#else
	char *id = argv[0], *class = argv[1], *detailID = argv[2], *size = argv[3], *title = argv[4], *duration = NULL,
             *bitrate = NULL, *sampleFrequency = NULL, *artist = NULL, *album = NULL, *genre = NULL,
             *comment = NULL, *date = NULL, *resolution = NULL, *mime = argv[5];
#endif
...
#else
#define SELECT_COLUMNS "SELECT o.OBJECT_ID, o.CLASS, o.DETAIL_ID, d.SIZE, d.TITLE, d.MIME "	
#endif
...
#else
		item = sql_get_text_field(db, "SELECT TITLE from OBJECTS o left join DETAILS d on "
									"(o.DETAIL_ID = d.ID) where OBJECT_ID = '%q'", objectID);
#endif
		if( item )
		{
