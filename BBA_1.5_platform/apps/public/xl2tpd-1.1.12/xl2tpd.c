...
int start_pppd (struct call *c, struct ppp_opts *opts)
{
...
#ifndef USE_FILE_TO_PASS_PPP_ARG
	if (gconfig.user[0])
	{
		stropt[pos++] = strdup("user");
		stropt[pos++] = strdup(gconfig.user);
	}
	if (gconfig.password[0])
	{
	...
	/*added by xianjiantao to make the l2tp sending lcp echo to check whether the peer is dead or not */
	stropt[pos++] = strdup("lcp-echo-interval");
	stropt[pos++] = strdup(gconfig.lcpEchoInt);

	stropt[pos++] = strdup("lcp-echo-failure");
	stropt[pos++] = strdup(gconfig.lcpEchoFailTime);
...
void init_args(int argc, char *argv[])
{
...
#else
	/* luodan add for special char */
	memset(gconfig.username_len, 0, STRLEN);
    memset(gconfig.password_len, 0, STRLEN);
	/* luodan add end */	
#endif /* USE_FILE_TO_PASS_PPP_ARG */
    ...
    /* end add */
	/*added by xianjiantao to make the l2tp sending lcp echo to check whether the peer is dead or not */
	memset(gconfig.lcpEchoInt, 0, STRLEN);
	memset(gconfig.lcpEchoFailTime, 0, STRLEN);
	/*added end xianjiantao*/
    ...
    for (i = 1; i < argc; i++) {
		...
		/*added by xianjiantao to make the l2tp sending lcp echo to check whether the peer is dead or not */
		else if (! strncmp(argv[i], "lcp-echo-interval", 17)) {
			if (++i == argc)
				usage();
			else
				strncpy(gconfig.lcpEchoInt, argv[i], sizeof(gconfig.lcpEchoInt) - 1);
		}
		else if (! strncmp(argv[i], "lcp-echo-failure", 16)) {
			if (++i == argc)
				usage();
			else
				strncpy(gconfig.lcpEchoFailTime, argv[i], sizeof(gconfig.lcpEchoFailTime) - 1);
		}
		/*added end xianjiantao*/
        /* end add */
        else {
...
{
#if 0 /* fix bug, we should match entry with longest subnet mask in routing table */
	char buf[256], dev[64];
	int metric, flags;
	u_int32_t dest, mask;
	...
	fclose(f);
#else
	char _buf[256], _dev[64], dev[64];
	int _metric, _flags, metric=100000;
	u_int32_t _dest, _mask, _s_addr, s_addr, mask=0;
	...
	while (fgets(_buf, sizeof(_buf), f)) 
	{
		if (sscanf(_buf, "%63s %x %x %X %*s %*s %d %x", _dev, &_dest,
		    	&_s_addr, &_flags, &_metric, &_mask) != 6)
			continue;
		if ((_flags & RTF_UP) == RTF_UP && (inetaddr.s_addr & _mask) == _dest &&
			(_mask > mask || (_mask==mask && _metric < metric)))
		{
			strcpy(dev, _dev);
			metric = _metric;
			mask = _mask;
			sin_addr(&rt->rt_gateway).s_addr=_s_addr;
			rt->rt_metric = _metric + 1;
			rt->rt_gateway.sa_family = AF_INET;
		}
	}
