...
#if defined(INCLUDE_USB_SYSFS)
static int sysfs_get_device_num(struct libusb_device *dev, int *majornum, int *minornum);
static int sysfs_create_device_node(struct libusb_device *dev, char *filename, int namelen);
#endif
...
#else
		snprintf(cmd, PATH_MAX, "rm -rf %s", DEV_AUTO_GEN_PATH);
		system(cmd);
		snprintf(cmd, PATH_MAX, "mkdir -m 0777 %s", DEV_AUTO_GEN_PATH);
		system(cmd);
#endif
	}
	...
	uint8_t config_index, unsigned char *buffer, size_t len, int *host_endian)
{
	...
	if (!usbfs_path)
	{
		r = sysfs_create_device_node(dev, filename, PATH_MAX);
		if (r < 0)
		{
			usbi_err(DEVICE_CTX(dev), "failed to create device from sysfs");
			return r;
		}
	}
	else
#endif /* defined(INCLUDE_USB_SYSFS) */
	{
...
static int op_open(struct libusb_device_handle *handle)
{
	...
	if (!usbfs_path)
	{
		if (sysfs_create_device_node(handle->dev, filename, PATH_MAX) < 0)
		{
			usbi_err(DEVICE_CTX(handle->dev), "failed to create device from sysfs");
			return LIBUSB_ERROR_OTHER;
		}
	}
	else
...
static int sysfs_get_device_num(struct libusb_device *dev, int *majornum, int *minornum)
{
	...
	usbi_dbg("");
	/* file name: "/sys/bus/usb/devices/usb1/dev", file content: "189:0" */
	snprintf(strarr, PATH_MAX, "%s/%s/%s", 
			SYSFS_DEVICE_PATH, __device_priv(dev)->sysfs_dir, "dev");
	usbi_dbg("start to read file %s", strarr);
	file = fopen(strarr, "r");
	if (file)
	{
		if (fgets(strarr, PATH_MAX, file))
		{
			pos = strarr;
			...
			*pos = '\0';
			sscanf(strarr, "%d", &major);
			sscanf(pos + 1, "%d", &minor);
		}
		else
		{
			usbi_err(DEVICE_CTX(dev), "Failed to fgets");
		}
		fclose(file);
	}
	else
	{
		usbi_err(DEVICE_CTX(dev), "Failed to read %s", strarr);
	}
	/* file name: "/sys/bus/usb/devices/usb1/uevent", 
	 * file content: MAJOR=189
	 * 				 MINOR=0
	...
	if (major == -1 || minor == -1)
	{
		...
		minor = -1;
		snprintf(strarr, PATH_MAX, "%s/%s/%s", 
				SYSFS_DEVICE_PATH, __device_priv(dev)->sysfs_dir, "uevent");
		usbi_dbg("start to read file %s", strarr);
		file = fopen(strarr, "r");
		if (file)
		{
			while (major == -1 || minor == -1)
			{
				if (fgets(strarr, PATH_MAX, file))
				{
					if (!strncmp(strarr, "MAJOR=", 6))
					{
						sscanf(strarr + 6, "%d", &major);
					}
					else if (!strncmp(strarr, "MINOR=", 6))
					{
						sscanf(strarr + 6, "%d", &minor);
					}
				}
				else
				{	
	...
	usbi_dbg("major = %d, minor = %d, busnum = %d, devnum=%d",
			major, minor, dev->bus_number, dev->device_address);
	if (major == -1 || minor == -1)
	{
		usbi_err(DEVICE_CTX(dev), "Failed to read major and minor");
		return LIBUSB_ERROR_OTHER;
	}
...
static int sysfs_create_device_node(struct libusb_device *dev, char *filename, int namelen)
{
	...
	usbi_dbg("");
	if (sysfs_get_device_num(dev, &major, &minor) < 0)
	{
		usbi_err(DEVICE_CTX(dev), "Failed to read major and minor");
		return LIBUSB_ERROR_OTHER;
	}
	...
	usbi_dbg("major = %d, minor = %d, busnum = %d, devnum=%d",
			major, minor, dev->bus_number, dev->device_address);
	/* delete device node: /var/dev/libusb/001/001 */
	snprintf(strarr, PATH_MAX, "rm -f %s/%03d/%03d", DEV_AUTO_GEN_PATH, dev->bus_number, dev->device_address);
	system(strarr);
	/* create folder: /var/dev/libusb/001 */
	snprintf(strarr, PATH_MAX, "mkdir -p -m 0777 %s/%03d", DEV_AUTO_GEN_PATH, dev->bus_number);
	system(strarr);
	/* create device node: /var/dev/libusb/001/001 */
	snprintf(strarr, PATH_MAX, "%s/%03d/%03d", DEV_AUTO_GEN_PATH, dev->bus_number, dev->device_address);
	if (mknod(strarr, S_IFCHR | 0666, MKDEV(major, minor)) < 0)
	{
		usbi_err(DEVICE_CTX(dev), "failed to mknod %s, %s", strarr, strerror(errno));
		return LIBUSB_ERROR_OTHER;
	}
	strncpy(filename, strarr, PATH_MAX);
	filename[namelen - 1] = 0;
	
	return LIBUSB_SUCCESS;
}

static int op_get_driver_name(struct libusb_device_handle *handle,
	int interface, char *name, unsigned int namelen)
{
	int fd = __device_handle_priv(handle)->fd;
	struct usbfs_getdriver getdrv;
	int r;
	...
	strncpy(name, getdrv.driver, namelen);
	name[namelen - 1] = 0;
	
	return 1;
}
#endif /* INCLUDE_USB_SYSFS */
