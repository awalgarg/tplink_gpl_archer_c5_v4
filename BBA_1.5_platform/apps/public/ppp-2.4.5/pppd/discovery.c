...
discovery(PPPoEConnection *conn)
{
...
#ifdef INCLUDE_PADT_BEFORE_DIAL
	/* 每次启动pppd进程，都尝试发PADT包终结上次的session. Added by Wang Jianfeng 2014-05-05*/
	/* save oldSession, Zhang Wenkai, 2016/9/27, sendTermination. */
	if (oldSession != 0xffff)
	{
		...
		vslp_printer(&bufinfo, "sent [PADT");
		if (conn->serviceName)
		{
			vslp_printer(&bufinfo, " Service-Name:%s", conn->serviceName);
		}
		...
		vslp_printer(&bufinfo, " Session-Id(0x%08x)", oldSession);
		vslp_printer(&bufinfo, "]");
		log_to_cmm(LOG_INFORM, "%s", buf);

		/* send Termination */
		sendTermination(conn, oldSession);
		memset(buf, 0, MAX_LOG_LEN);
		bufinfo.ptr = buf;
		bufinfo.len = MAX_LOG_LEN;

		vslp_printer(&bufinfo, "sent [Termination");
		if (conn->serviceName)
		{
			vslp_printer(&bufinfo, " Service-Name:%s", conn->serviceName);
		}
...
#ifdef INCLUDE_PADT_BEFORE_DIAL
	    /* Added by Zhang Wenkai, 2016/10/13, only try more than MAX_PADI_ATTEMPTS, then reset oldSession to 0. */
	    oldSession = 0xffff;
#endif
