...
#if 1
	/* Solve Bug 124455, by zdb, 20160510. 
	 * interpret_string_addr_internal will always fail as we didn't register our dut name to the 
	 * DNS server. And when we had set dns server address in system(if wan is connected), 
	 * getaddrinfo will block umpty seconds before return. For some client systems(like xp),  
	 * this will cause timeout. So far, for our dut, we just use samba in a local network, it 
	 * will be ok to return false here.
	 */
	ret = false;
#else
