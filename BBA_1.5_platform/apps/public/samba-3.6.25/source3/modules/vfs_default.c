...
static int vfswrap_ftruncate(vfs_handle_struct *handle, files_struct *fsp, SMB_OFF_T len)
{
...
#ifdef INCLUDE_USB_STORAGE_TUXERA
	struct statfs fs_buf;
	fstatfs(fsp->fh->fd, &fs_buf);

	/* we do not preallocate space for FAT system, instead give a special handle here. 
	 * 	The issue is that we increase disk fragment by doing so---wubaijun
	 */
	if(fs_buf.f_type == 0x4d44){	
		...
		space_avail = (double)((double)(fs_buf.f_bsize/1024) * (double)fs_buf.f_bavail);

		DEBUG(4,("avail space is %f, block size %d\n", space_avail, fs_buf.f_bsize));
		...
		if((double)(len/1024) > space_avail){	//instead we just check if the file space needed is bigger than the available space on usb---wubaijun
			DEBUG(4,("file_len:%d\tno space: %d\n", (long)(len/1024), space_avail));
			errno = ENOSPC;
			goto done;
		}
		...
		if((double)(len/1024) < space_avail){	//shrink the file size should work fine for FAT---wubaijun
			...
			if(pst->st_ex_size <= len){	//do not ftruncate if we need to expand the size
				result = 0;
				goto done;
			}
	...
	if (result == 0)
		goto done;
#else
	/*	modified by gengxianhu for no pre-allocating space
	if (result == 0)
		goto done; */

	/* vfat return EPERM, ntfs error remain */
	if (result != 0 && errno == EPERM) 
	{
		errno = EINVAL;
		result = 0;
	/* 
		currently, we only support FAT32/NTFS/EXT2 fs
		only FAT32 don't support ftruncate operation and 4GB larger files
		our SMB_OFF_T is 8 bytes, for preventing error of sending file
		which is larger than 4GB,so we check it here
		although ntfs or ext2 can also reach here, most reason is DISKFULL
		we also send it back to PC by samba error process
		added by gengxianhu
	 */
	...
	goto done;	/* don't support write 0 at end of file for pre-allocating space */
#endif
