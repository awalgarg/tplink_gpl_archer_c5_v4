...
/* added by tiger 03/27/09, set passwd from cmdline */
static char *cmd_passwd = NULL;
...
static void usage(void)
{
	...
	/* added by tiger 03/27/09, set passwd from cmdline */
	printf("  -p PASSWORD          set user passwd\n");
...
static int process_options(int argc, char **argv, int local_flags)
{
	...
	/* modified by tiger 03/27/09, set passwd from cmdline */
	while ((ch = getopt(argc, argv, "c:axdehminjr:sw:R:D:U:LWp:")) != EOF) {
		switch(ch) {
		case 'L':
		...
		/* added by tiger 03/27/09, set passwd from cmdline */
		/* it is used without -s option */
		case 'p': {
			local_flags |= LOCAL_CMD_PASSWORD;
			cmd_passwd = smb_xstrdup(optarg);
			break;
		}
		case 'h':
		default:
...
static int process_root(int local_flags)
{
	...
	if (local_flags & LOCAL_TRUST_ACCOUNT) {
		...
		if((local_flags & LOCAL_SET_PASSWORD) && (new_passwd == NULL)) {
			/* modifed by tiger 03/27/09, set password by cmdline */
			if (local_flags & LOCAL_CMD_PASSWORD) {
				new_passwd = cmd_passwd;
			} else {
			new_passwd = prompt_for_new_password(stdin_passwd_get);
			}
			if(!new_passwd) {
