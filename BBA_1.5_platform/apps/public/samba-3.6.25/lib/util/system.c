...
{
	/* Modifed by zjj, 20131215. 
	 * The original code will call gethostbyname to get the ip addrs for the dut
	 * name(like Archer D7). After dialing up, we will set the DNS server by 
	 * modifying /etc/resolv.conf, while we will not register our dut name to the 
	 * DNS server. As a result, this function will fail at last, besides, 
	 * gethostbyname will block 30 seconds before return. 
	 * For a user log on, this function will be called 8 times, that means the
	 * user will wait at least 4 minutes before he gets the result. For some client
	 * systems(like xp), this will cause timeout.
	 * So far, for our dut, we just use samba in a local network, it will be ok to 
	 * return NULL here.
	 */
	return NULL;
