...
/* Port from busybox by yangxv <yangxu@tp-link.com.cn>, 2011.10.9 */
...
"Overview\n"
"--------\n"
"Some features may be built directly into Lettuce.  Some features\n"
...
load_config_help[] =
	"\n"
	"For various reasons, one may wish to keep several different Lettuce\n"
	"configurations available on a single machine.\n"
	"\n"
	"If you have saved a previous configuration in a file other than the\n"
	"Lettuce's default, entering the name of the file here will allow you\n"
...
save_config_help[] =
	"\n"
	"For various reasons, one may wish to keep different Lettuce\n"
	"configurations available on a single machine.\n"
	"\n"
...
int main(int ac, char **av)
{
	...
	if (ac >= 3 && strncmp(av[2], "silent", sizeof("silent")) == 0)
		silent = 1;
	...
	sym_calc_value(sym);
	snprintf(menu_backtitle, 128, "Lettuce v%s Configuration",
		sym_get_string_value(sym));
	...
	do {
		stat = dialog_yesno(NULL,
				    "Do you wish to save your new Lettuce configuration?", 5, 60);
	} while (stat < 0);
	end_dialog();
	}
	else
	...
	if (stat == 0) {
		conf_write(NULL);
		printf("\n\n"
			"*** End of Lettuce configuration.\n"
			"*** Check the top-level Makefile for additional configuration options.\n\n");
	} else
		printf("\n\nYour Lettuce configuration changes were NOT saved.\n\n");
