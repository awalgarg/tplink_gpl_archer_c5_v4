...
int main(int argc, char **argv)
{
	...
	sprintf(image_name_prefix, "%s/%sv%d_%d.%d.0_%d.%d.%d",
					image_path, prefix, 
					strtoul(l_dev_info.dev_ver, 0, 0),
	...
	sprintf(bin_file, "%s_flash(%s).bin", image_name_prefix, l_dev_info.build_date);
	sprintf(update_file, "%s_up(%s).bin", image_name_prefix, l_dev_info.build_date);
	sprintf(update_file_boot, "%s_up_boot(%s).bin", image_name_prefix, l_dev_info.build_date);
	...
	if (1 == dual_image) 
	{
		sprintf(bin_file, "%s_flash_%dM(%s).bin", image_name_prefix, flash_size/0x100000, l_dev_info.build_date);
