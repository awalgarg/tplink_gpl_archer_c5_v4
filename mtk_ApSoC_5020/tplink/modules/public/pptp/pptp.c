...
#else            
        if ((IS_ERR(ip_route_output_key(&init_net, &fl.u.ip4)))) {
#endif
			goto tx_error;
		}
	}
...
#else    
    if (!po->chan.mtu) po->chan.mtu=PPP_MRU; /* no PPP_MTU defined ? */
#endif    
#endif
	ip_rt_put(rt);
...
#if defined(CONFIG_NET_IPGRE) && defined(INCLUDE_TTNET)
	if (inet_add_protocol(&net_pptp_protocol, IPPROTO_PPTP) < 0) {
		printk("can't add protocol IPPROTO_PPTP\n");
#else
...
#if defined(CONFIG_NET_IPGRE) && defined(INCLUDE_TTNET)
	inet_del_protocol(&net_pptp_protocol, IPPROTO_PPTP);
#else
