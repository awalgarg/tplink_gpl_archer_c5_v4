			    ...
			    const u8 *frame, size_t len)
{
	...
	if (status != WLAN_STATUS_SUCCESS) {
		...
		printf("YF DEUBG: mlme_event_assoc ===> EVENT_ASSOC_REJECT");
			       ...
			       struct nlattr *resp_ie)
{
	...
	if (cmd == NL80211_CMD_CONNECT &&
		...
		printf("YF DEUBG: mlme_event_connect ==> EVENT_ASSOC_REJECT\n");
		wpa_supplicant_event(drv->ctx, EVENT_ASSOC_REJECT, &event);
		return;
	}
	...
	printf("YF DEUBG: mlme_event_connect ==> EVENT_ASSOC_OK\n");
	wpa_supplicant_event(drv->ctx, EVENT_ASSOC, &event);
}
			    ...
			    struct nlattr *freq, const u8 *frame, size_t len)
{
	...
	if (stype == WLAN_FC_STYPE_ACTION) {
		...
		wpa_printf(MSG_DEBUG, "-->mlme_event_mgmt(0)\n");
				      ...
				      struct nlattr **tb)
{
	...
	wpa_printf(MSG_ERROR, "nl80211: New station " MACSTR, MAC2STR(addr));
	...
	if (is_ap_interface(drv->nlmode) && drv->device_ap_sme) {
		...
		wpa_hexdump(MSG_ERROR, "nl80211: Assoc Req IEs", ies, ies_len);
...
broken_combination:
	...
	if (tb[NL80211_ATTR_OFFCHANNEL_TX_OK]) {
		printf("nl80211: Using driver-based "
			   "off-channel TX\n");
...
//	drv->use_monitor = 0;
	...
	//printf("1.drv->device_ap_sme: %d, drv->use_monitor: %d\n", drv->device_ap_sme, drv->use_monitor);
...
static int nl80211_setup_ap(struct i802_bss *bss)
{
	...
	if (drv->device_ap_sme &&
		...
		wpa_printf(MSG_ERROR, "nl80211: Failed to enable "
			   "Probe Request frame reporting in AP mode");
		/* Try to survive without this */
	}

	
        wpa_printf(MSG_ERROR, "nl80211: Setup AP Comple- device_ap_sme=%d "
                   "use_monitor=%d", drv->device_ap_sme, drv->use_monitor);
						...
						unsigned int duration)
{
	...
	u64 cookie;
		...
		wpa_printf(MSG_DEBUG, "wpa_driver_nl80211_remain_on_channel "
			   "freq=%u MHz duration=%u", freq, duration);
