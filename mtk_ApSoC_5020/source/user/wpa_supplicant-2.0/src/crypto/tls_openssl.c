				  ...
				  size_t ca_cert_blob_len, const char *ca_path)
{
	...
	if (ca_cert || ca_path) {
		...
		if (SSL_CTX_load_verify_locations(ssl_ctx, ca_cert, ca_path) !=
			...
			wpa_printf(MSG_DEBUG, "TLS: (1)Trusted root "
...
static int tls_global_ca_cert(SSL_CTX *ssl_ctx, const char *ca_cert)
{
	if (ca_cert) {
		...
		wpa_printf(MSG_DEBUG, "TLS: (2)Trusted root "
			   "certificate(s) loaded");
			      ...
			      const struct tls_connection_params *params)
{
	...
	while ((err = ERR_get_error())) {
		wpa_printf(MSG_INFO, "%s: woody Clearing pending SSL error: %s",
	...
	{
		wpa_printf(MSG_INFO, "%s: ERROR2",__func__);
		return -1;
	}
	wpa_printf(MSG_INFO, "%s: GG2",__func__);
	...
	wpa_printf(MSG_INFO, "%s: GG3",__func__);
	...
	wpa_printf(MSG_INFO, "%s: GG4",__func__);
	return 0;
}
