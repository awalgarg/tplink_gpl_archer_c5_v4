			      ...
			      struct wpa_bss *bss, struct wpa_ssid *ssid)
{
	...
	if (bss && !wpas_driver_bss_selection(wpa_s)) {
		...
		const u8 *ie, *md = NULL;
#endif /* CONFIG_IEEE80211R */
		wpa_msg(wpa_s, MSG_DEBUG, "Trying to associate with " MACSTR
						 ...
						 struct wpa_interface *iface)
{
	...
	wpa_dbg(wpa_s, MSG_DEBUG, "wpa_supplicant_add_iface-->wpa_s->conf->p2p_listen_reg_class (%d)", wpa_s->conf->p2p_listen_reg_class);
	return wpa_s;
}
