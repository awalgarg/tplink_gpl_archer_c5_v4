...
int wpas_p2p_init(struct wpa_global *global, struct wpa_supplicant *wpa_s)
{
	...
	if (!wpa_s->conf->p2p_listen_reg_class)
		wpa_printf(MSG_ERROR, "NG1");
	if (!wpa_s->conf->p2p_listen_channel)
		wpa_printf(MSG_ERROR, "NG2");
	
	if (wpa_s->conf->p2p_listen_reg_class &&
	    wpa_s->conf->p2p_listen_channel) {
		p2p.reg_class = wpa_s->conf->p2p_listen_reg_class;
		p2p.channel = wpa_s->conf->p2p_listen_channel;
		wpa_printf(MSG_ERROR, "P2P: Own listen channel(1): channel=%d reg_class=%d", p2p.channel, p2p.reg_class);
	} else {
		p2p.reg_class = 81;
		/*
		...
		p2p.channel = 1 + (r % 3) * 5;
		wpa_printf(MSG_ERROR, "P2P: Own listen channel(2): %d", p2p.channel);
	}
	wpa_printf(MSG_DEBUG, "P2P: Own listen channel: %d", p2p.channel);
	wpa_printf(MSG_ERROR, "P2P: Own listen channel: %d", p2p.channel);
	...
	if (wpa_s->conf->p2p_oper_reg_class &&
		...
		wpa_printf(MSG_ERROR, "P2P: Configured operating channel: "
		...
		wpa_printf(MSG_ERROR, "P2P: Random operating channel: "
			   "%d:%d", p2p.op_reg_class, p2p.op_channel);
	}
...
void wpas_p2p_update_config(struct wpa_supplicant *wpa_s)
{
	...
	wpa_printf(MSG_ERROR, "wpas_p2p_update_config");
