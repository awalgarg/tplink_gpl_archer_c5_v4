...
***************************************
 $Author: jianghua.qin $
 $Id: //WIFI_SOC/MP/SDK_5_0_0_0/RT288x_SDK/source/user/ecmh/src/ecmh.c#1 $
 $Date: 2015/03/04 $
***************************************
...
void l2_ethtype(struct intnode *intn, const uint8_t *packet, const unsigned int len, const unsigned int ether_type, unsigned char *src_mac);
void l2_eth(struct intnode *intn, struct ether_header *eth, const unsigned int len);
void icmp6_send(struct intnode *intn, const struct in6_addr *src, unsigned char *src_mac, int type, int code, void *data, unsigned int dlen);
...
/* Send a packet */
void sendpacket6(struct intnode *intn, struct intnode *input_intn, int ucast_f, unsigned char *src_mac, const struct ip6_hdr *iph, const uint16_t len);
void sendpacket6(struct intnode *intn, struct intnode *input_intn, int ucast_f, unsigned char *src_mac, const struct ip6_hdr *iph, const uint16_t len)
{
	int     sent;
#ifndef ECMH_BPF
	 ...
	 */
	if(ucast_f && src_mac){
		sa.sll_addr[0] = src_mac[0];
		sa.sll_addr[1] = src_mac[1];
		sa.sll_addr[2] = src_mac[2];
		sa.sll_addr[3] = src_mac[3];
		sa.sll_addr[4] = src_mac[4];
		sa.sll_addr[5] = src_mac[5];
	}else{
		sa.sll_addr[0] = 0x33;
		sa.sll_addr[1] = 0x33;
	...
	if (sent < 0)
	{
		...
		if (errno == ENXIO)
		{
			...
			/* Ralink: for ipv6 ready logo "Packet Too Big" */
			if(errno == EMSGSIZE && input_intn && src_mac && !ucast_f /* avoid recursion */){
				struct icmp6_hdr *icmpv6 = (void *)iph + sizeof(struct ip6_hdr);
				unsigned int plen = ntohs(iph->ip6_plen);
				int new_mtu = getMTU(intn);

				if(new_mtu == -1)
					return;
				plen = plen > 64 ? 64 : plen;	/* payload is stripped. */
				icmpv6->icmp6_mtu = htonl(new_mtu);
				dolog(LOG_DEBUG, "sending icmp \"Packet Too Big\" to %02x:%02x:%02x:%02x:%02x:%02x, plen=%d, act len = %d\n", src_mac[0],src_mac[1],src_mac[2],src_mac[3],src_mac[4],src_mac[5], plen, plen - sizeof(*icmpv6)+sizeof(icmpv6->icmp6_data32));
				icmp6_send(input_intn, &iph->ip6_src, src_mac, ICMP6_PACKET_TOO_BIG, 0, &icmpv6->icmp6_data32, plen - sizeof(*icmpv6)+sizeof(icmpv6->icmp6_data32));
			}
		}
		return;
	}
...
void icmp6_send(struct intnode *intn, const struct in6_addr *src, unsigned char *src_mac, int type, int code, void *data, unsigned int dlen)
{
	...
	sendpacket6(intn, NULL, type == ICMP6_PACKET_TOO_BIG ? 1 : 0, src_mac, (const struct ip6_hdr *)&packet, sizeof(packet) - (sizeof(packet.data) - dlen) - sizeof(packet.icmp6.icmp6_data32));
...
#endif
	sendpacket6(intn, NULL, 0, NULL, (const struct ip6_hdr *)&packet, packetlen);
...
void mld1_send_report(struct intnode *intn, const struct in6_addr *mca)
{
	...
	sendpacket6(intn, NULL, 0, NULL, (const struct ip6_hdr *)&packet, sizeof(packet));
		...
		LIST_LOOP(groupn->interfaces, grpintn, gn)
		{
			...
			LIST_LOOP(grpintn->subscriptions, subscrn, sn)
			{
					   ...
					   (any || IN6_IS_ADDR_UNSPECIFIED(&subscrn->ipv6) ? 0 : sizeof(*src))) > intn->mtu)
				{
					...
					sendpacket6(intn, NULL, 0, NULL, (const struct ip6_hdr *)packet, length + sizeof(packet->ip6));
...
void l4_ipv4_proto41(struct intnode *intn, struct ip *iph, const uint8_t *packet, const uint16_t len)
{
	...
	l2_ethtype(tun, packet, len, ETH_P_IPV6, NULL);
	return;
}
#endif /* ECMH_BPF */
...
void l4_ipv6_icmpv6_mld1_report(struct intnode *intn, const struct ip6_hdr *iph, const uint16_t len, struct mld1 *mld1, const uint16_t plen, unsigned char *src_mac);
void l4_ipv6_icmpv6_mld1_report(struct intnode *intn, const struct ip6_hdr *iph, const uint16_t len, struct mld1 *mld1, const uint16_t plen, unsigned char *src_mac)
{
	struct grpintnode	*grpintn;
	struct in6_addr		any, ipv6addr;
	bool			isnew;
	...
	if (	!IN6_IS_ADDR_MULTICAST(&mld1->mca) ||
		...
		IN6_IS_ADDR_MC_RESERVED_0(&mld1->mca)){	/* Ralink: for ipv6 ready logo. RFC4291*/
		mld_log(LOG_WARNING, "Skip new group %s for %s/%u (RFC4291)\n", &mld1->mca, intn);
		return;
	}
	...
	/* Ralink: copy source ipv6 address*/
	if(g_conf->es){
		...
		if(isnew){
			...
			inet_ntop(AF_INET6, &mld1->mca, buf, sizeof(buf));
			sprintf(cmd, "iwpriv ra0 set IgmpAdd=%s", buf);
			dolog(LOG_DEBUG, ">>>> == %s\n", cmd);
			system(cmd);
		}
#endif
		memcpy(&any, &iph->ip6_src, sizeof(any));
		if (!grpint_refresh_snooping(grpintn, &mld1->mca, &any, src_mac, MLD2_MODE_IS_INCLUDE, 1))	{
...
void l4_ipv6_icmpv6_mld1_reduction(struct intnode *intn, const struct ip6_hdr *iph, const uint16_t len, struct mld1 *mld1, const uint16_t plen, unsigned char *src_mac);
void l4_ipv6_icmpv6_mld1_reduction(struct intnode *intn, const struct ip6_hdr *iph, const uint16_t len, struct mld1 *mld1, const uint16_t plen, unsigned char *src_mac)
{
	struct groupnode	*groupn;
	struct grpintnode	*grpintn;
	...
	if(g_conf->es){
		...
		memset(buf, 0, sizeof(buf));
                inet_ntop(AF_INET6, &mld1->mca, buf, sizeof(buf));
                sprintf(cmd, "iwpriv ra0 set IgmpDel=%s-%s", buf, src_mac);
                dolog(LOG_DEBUG, ">>>> delete ==>>> %s\n", cmd);
                system(cmd);
#endif
		memcpy(&any, &iph->ip6_src, sizeof(any));
	}
	...
	if (!subscr_unsub(grpintn->subscriptions, &any))
	{
		...
		update_group_port_map(&mld1->mca, MACTABLE_ENTRY_ZEROED);
...
#ifdef ECMH_SUPPORT_MLD2
void l4_ipv6_icmpv6_mld2_report(struct intnode *intn, const struct ip6_hdr *iph, const uint16_t len, struct mld2_report *mld2r, const uint16_t plen, unsigned char *src_mac);
void l4_ipv6_icmpv6_mld2_report(struct intnode *intn, const struct ip6_hdr *iph, const uint16_t len, struct mld2_report *mld2r, const uint16_t plen, unsigned char *src_mac)
{
	...
	while (ngrec > 0)
	{
		...
		if (nsrcs == 0)
		{
			if (grpintn)
			{
				...
				if(g_conf->es){
					memcpy(&any, &iph->ip6_src, sizeof(any));
					if (!grpint_refresh_snooping(grpintn, &grec->grec_mca, &any, src_mac,
						grec->grec_type == MLD2_MODE_IS_EXCLUDE ||
						grec->grec_type == MLD2_CHANGE_TO_EXCLUDE ||
						grec->grec_type == MLD2_BLOCK_OLD_SOURCES ?
						MLD2_MODE_IS_INCLUDE : MLD2_MODE_IS_EXCLUDE, 1))
					{
		...
		else
		{
			...
			while (nsrcs > 0)
			{
				...
				if (grpintn)
				{
					...
					/* Ralink: copy source ipv6 address*/
					if(g_conf->es){
						if (!grpint_refresh_snooping(grpintn, &grec->grec_mca, src, src_mac,
							grec->grec_type == MLD2_MODE_IS_EXCLUDE ||
							grec->grec_type == MLD2_CHANGE_TO_EXCLUDE ||
							grec->grec_type == MLD2_BLOCK_OLD_SOURCES ?
							MLD2_MODE_IS_EXCLUDE : MLD2_MODE_IS_INCLUDE, 0))
						{
 ...
 */
void l4_ipv6_multicast(struct intnode *intn, struct ip6_hdr *iph, const uint16_t len, unsigned char *src_mac);
void l4_ipv6_multicast(struct intnode *intn, struct ip6_hdr *iph, const uint16_t len, unsigned char *src_mac)
{
...
*/
	...
	if (IN6_IS_ADDR_MULTICAST(&iph->ip6_src)){
		inet_ntop(AF_INET6, &iph->ip6_src, src, sizeof(src));
		inet_ntop(AF_INET6, &iph->ip6_dst, dst, sizeof(dst));
		dolog(LOG_WARNING, "%5s, SRC is multicast addr. L3:IPv6: IPv%0x %40s %40s %4u %u\n", intn->name, (int)((iph->ip6_vfc>>4)&0x0f), src, dst, ntohs(iph->ip6_plen), iph->ip6_nxt);
		return;
	}

	if (IN6_IS_ADDR_UNSPECIFIED(&iph->ip6_src)){
		inet_ntop(AF_INET6, &iph->ip6_src, src, sizeof(src));
		inet_ntop(AF_INET6, &iph->ip6_dst, dst, sizeof(dst));
		dolog(LOG_WARNING, "%5s, SRC is unspecified. L3:IPv6: IPv%0x %40s %40s %4u %u\n", intn->name, (int)((iph->ip6_vfc>>4)&0x0f), src, dst, ntohs(iph->ip6_plen), iph->ip6_nxt);
		return;
	}

	if (IN6_IS_ADDR_UNSPECIFIED(&iph->ip6_dst)){
		inet_ntop(AF_INET6, &iph->ip6_src, src, sizeof(src));
		inet_ntop(AF_INET6, &iph->ip6_dst, dst, sizeof(dst));
		dolog(LOG_WARNING, "%5s, DST is unspecified. L3:IPv6: IPv%0x %40s %40s %4u %u\n", intn->name, (int)((iph->ip6_vfc>>4)&0x0f), src, dst, ntohs(iph->ip6_plen), iph->ip6_nxt);
		return;
	}
	...
	if (IN6_IS_ADDR_LINKLOCAL(&iph->ip6_src)){
		inet_ntop(AF_INET6, &iph->ip6_src, src, sizeof(src));
		inet_ntop(AF_INET6, &iph->ip6_dst, dst, sizeof(dst));
		dolog(LOG_WARNING, "%5s, SRC is link-local. L3:IPv6: IPv%0x %40s %40s %4u %u\n", intn->name, (int)((iph->ip6_vfc>>4)&0x0f), src, dst, ntohs(iph->ip6_plen), iph->ip6_nxt);
		return;
	}

	if (IN6_IS_ADDR_MC_NODELOCAL(&iph->ip6_dst)){
		inet_ntop(AF_INET6, &iph->ip6_src, src, sizeof(src));
		inet_ntop(AF_INET6, &iph->ip6_dst, dst, sizeof(dst));
		dolog(LOG_WARNING, "%5s, DST is node-local multicast. L3:IPv6: IPv%0x %40s %40s %4u %u\n", intn->name, (int)((iph->ip6_vfc>>4)&0x0f), src, dst, ntohs(iph->ip6_plen), iph->ip6_nxt);
		return;
	}

	if (IN6_IS_ADDR_MC_LINKLOCAL(&iph->ip6_dst)){
		inet_ntop(AF_INET6, &iph->ip6_src, src, sizeof(src));
		inet_ntop(AF_INET6, &iph->ip6_dst, dst, sizeof(dst));
		dolog(LOG_WARNING, "%5s, DST is link-local multicast. L3:IPv6: IPv%0x %40s %40s %4u %u\n", intn->name, (int)((iph->ip6_vfc>>4)&0x0f), src, dst, ntohs(iph->ip6_plen), iph->ip6_nxt);
		return;
	}
	...
	LIST_LOOP(groupn->interfaces, grpintn, in)
	{
		...
		LIST_LOOP(grpintn->subscriptions, subscrn, in2)
		{
			...
			if (	!IN6_ARE_ADDR_EQUAL(&subscrn->ipv6, &in6addr_any) &&
				...
				!subscrn->any_address_flag){	/* ralink */
				continue;
			}
			...
			sendpacket6(interface, intn, 0, src_mac, iph, len);
 ...
 */
void l4_ipv6_icmpv6(struct intnode *intn, struct ip6_hdr *iph, const uint16_t len, struct icmp6_hdr *icmpv6, const uint16_t plen, unsigned char *src_mac);
void l4_ipv6_icmpv6(struct intnode *intn, struct ip6_hdr *iph, const uint16_t len, struct icmp6_hdr *icmpv6, const uint16_t plen, unsigned char *src_mac)
{
	uint16_t		csum;
	...
	if (icmpv6->icmp6_type == ICMP6_ECHO_REQUEST)
	{
		...
		if (iph->ip6_hlim == 0)
		{
			...
			icmp6_send(intn, &iph->ip6_src, src_mac, ICMP6_ECHO_REPLY, ICMP6_TIME_EXCEED_TRANSIT, &icmpv6->icmp6_data32, plen-sizeof(*icmpv6)+sizeof(icmpv6->icmp6_data32));
			return;
		}
		/* Send this packet along it's way */
		else l4_ipv6_multicast(intn, iph, len, src_mac);
	}
	else
	{
		if (!(IN6_IS_ADDR_LINKLOCAL(&iph->ip6_src)))
		...
		if (icmpv6->icmp6_type == ICMP6_MEMBERSHIP_REPORT)
		{
			l4_ipv6_icmpv6_mld1_report(intn, iph, len, (struct mld1 *)icmpv6, plen, src_mac);
		}
		else if (icmpv6->icmp6_type == ICMP6_MEMBERSHIP_REDUCTION)
		{
			l4_ipv6_icmpv6_mld1_reduction(intn, iph, len, (struct mld1 *)icmpv6, plen, src_mac);
		}
#ifdef ECMH_SUPPORT_MLD2
		else if (icmpv6->icmp6_type == ICMP6_V2_MEMBERSHIP_REPORT ||
			 icmpv6->icmp6_type == ICMP6_V2_MEMBERSHIP_REPORT_EXP)
		{
			l4_ipv6_icmpv6_mld2_report(intn, iph, len, (struct mld2_report *)icmpv6, plen, src_mac);
		}
#endif /* ECMH_SUPPORT_MLD2 */
...
void l3_ipv6(struct intnode *intn, struct ip6_hdr *iph, const uint16_t len, unsigned char *src_mac);
void l3_ipv6(struct intnode *intn, struct ip6_hdr *iph, const uint16_t len, unsigned char *src_mac)
{
	struct ip6_ext		*ipe;
	uint8_t			ipe_type;
...
/* Ralink, ipv6 ready logo spec.p2 #53 #54 */
/* from RFC2460/2780 which is updated 2011-02-03 */
#define IPV6_HOPBYHOP_OPTION_SKIP			0
#define IPV6_HOPBYHOP_OPTION_DISCARD			1
#define IPV6_HOPBYHOP_OPTION_DISCARD_SENDICMP		2
#define IPV6_HOPBYHOP_OPTION_DISCARD_SENDICMP_UNICAST	3
		if(ipe_type == IPPROTO_HOPOPTS){
			int option_type;
			unsigned char *option = (unsigned char *)ipe + sizeof(struct ip6_ext);
			option_type = (*option & 0xc0) >> 6;
			if(option_type == IPV6_HOPBYHOP_OPTION_DISCARD ||
			   option_type == IPV6_HOPBYHOP_OPTION_DISCARD_SENDICMP ||
			   option_type == IPV6_HOPBYHOP_OPTION_DISCARD_SENDICMP_UNICAST){
				dolog(LOG_DEBUG, "Skipping packet with specific Hop-by-Hop option on %s\n", intn->name);
				return;		
			}
		}
/* Ralink */
	...
	{
		/* Take care of ICMPv6 */
		l4_ipv6_icmpv6(intn, iph, len, (struct icmp6_hdr *)ipe, plen, src_mac);
		return;
	}
	...
	if (IN6_IS_ADDR_MULTICAST(&iph->ip6_dst))
	{
		...
		l4_ipv6_multicast(intn, iph, len, src_mac);
		return;
	}
...
void l2_ethtype(struct intnode *intn, const uint8_t *packet, const unsigned int len, const unsigned int ether_type, unsigned char *src_mac)
{
	...
	{
		l3_ipv6(intn, (struct ip6_hdr	*)packet, len, src_mac);
	}
...
void l2_eth(struct intnode *intn, struct ether_header *eth, const unsigned int len)
{
	l2_ethtype(intn, ((uint8_t *)eth + sizeof(*eth)), len-sizeof(*eth), ntohs(eth->ether_type), NULL);
}
...
void update_interfaces(struct intnode *intn)
{
	...
	if ( last_update && last_update+(5*60) > time(NULL) ) {
		return;
	}
	...
	if (getifaddrs(&ifap) == 0)
	{
		for (ifa=ifap;ifa;ifa=ifa->ifa_next)
		{
			...
			for(i=0; g_conf->positive_intf[i]; i++){
#ifndef ECMH_GETIFADDR
				if(!strcmp(g_conf->positive_intf[i], devname)){
#else
				if(!strcmp(g_conf->positive_intf[i], ifa->ifa_name)){
#endif
					ignore = false;
				}
			}
...
void init(void)
{
	...
	g_conf->positive_intf[0]	= DEFAULT_INTERFACE1;
	g_conf->positive_intf[1]	= DEFAULT_INTERFACE2;
	g_conf->positive_intf[2]	= NULL;
	g_conf->positive_intf[3]	= NULL;
	g_conf->positive_intf[4]	= NULL;
	g_conf->positive_intf[5]	= NULL;
	g_conf->positive_intf[6]	= NULL;
	g_conf->positive_intf[7]	= NULL;
}
	...
	LIST_LOOP(g_conf->groups, groupn, ln)
	{
		...
		LIST_LOOP(groupn->interfaces, grpintn, gn)
		{
			...
			LIST_LOOP(grpintn->subscriptions, subscrn, ssn)
			{
				...
				fprintf(g_conf->stat_file, "\t\t%s portnum:%d %s (%u seconds old)\n",
					addr,
					subscrn->portnum,
...
void subscription_timeout(void);
void subscription_timeout(void)
{
	struct groupnode	*groupn;
	struct listnode		*ln, *ln2;
	...
	struct in6_addr		changed_groups[MAX_MULTICASTS_GROUP];
	int			deleted_groups[MAX_MULTICASTS_GROUP] = {0};
	int			changed_groups_count = 0, i;

	/* ralink, multiple timers management. */
	static int count = 0;
	count += RALINK_TIMEOUT;
	if(count < ECMH_SUBSCRIPTION_TIMEOUT)
		return;
	count = 0;

	dolog(LOG_DEBUG, "Subscription Timeout occur.\n");
	...
	LIST_LOOP2(g_conf->groups, groupn, ln, ln2)
	{
		...
		if (groupn->interfaces->count == 0)
		{
			...
			group_changed = true;
#ifdef WIFI_IGMPSNOOP_SUPPORT
			deleted_groups[changed_groups_count] = 1;
#endif
		}

		/* Ralink, TODO: search previous the same group */
		if(group_changed && count < MAX_MULTICASTS_GROUP)
			memcpy(&changed_groups[changed_groups_count++], &groupn->mca, sizeof(struct in6_addr));
	}
	LIST_LOOP2_END

	/* Ralink, remove entry from mac table */
	for(i=0; i<changed_groups_count; i++){
		char buf[1024];
		inet_ntop(AF_INET6, &changed_groups[i], (char *)&buf, sizeof(buf));
		dolog(LOG_DEBUG, "*** Ralink removing %s...\n", buf);

#ifdef WIFI_IGMPSNOOP_SUPPORT
		if(deleted_groups[i]){
			char cmd[1024];
			int grpnum = GetGroup(&changed_groups[i]);
			...
			/* This is the lastest group entry */
			sprintf(cmd, "iwpriv ra0 set IgmpDel=%s", buf);
			dolog(LOG_DEBUG, ">>> delete --> %s", cmd);
			system(cmd);
		}
#endif
		update_group_port_map(&changed_groups[i], MACTABLE_ENTRY_DELETED);
	}
...
bool handleinterfaces(uint8_t *buffer)
{
	...
	if(g_conf->es){
		/* Ralink: What we want to know */
		FD_SET(g_conf->rawsocket, &fd_read);
		memset(&timeout, 0, sizeof(timeout));
		timeout.tv_sec = 5;

		i = select(g_conf->rawsocket+1, &fd_read, NULL, NULL, &timeout);
		if (i < 0){
			if (errno == EINTR)
				return true;
	...
	if (intn)
	{
		...
		/* Handle the packet */
		l2_ethtype(intn, buffer, len, ntohs(sa.sll_protocol), sa.sll_addr);
	}else{
		/* Ralink */
...
void clear_intfs(void)
{
	int i;
	for(i=0; i< MAX_INTERFACES; i++)
		g_conf->positive_intf[i] = NULL;
}

int parse_intfs(char *str)
{
	int count = 0;

        char *tok;
        tok = strtok(str, " ");
        while(tok && count != MAX_INTERFACES){
		g_conf->positive_intf[count] = tok;
		printf("%s is added to multicast forwarding.\n", tok);
		count++;
                tok = strtok(NULL, " ");
        }
...
#endif
	{"interfaces",		no_argument,		NULL, 'I'},
	{"enable_snooping",	no_argument,		NULL, 'e'},
	{NULL,			0, NULL, 0},
};
...
int main(int argc, char *argv[])
{
	...
	/* Handle arguments */
	while ((i = getopt_long(argc, argv, "dfi:pPu:eI:"
#ifdef ECMH_BPF
		"tT"
#endif
		"vV"
		...
		, long_options, &option_index)) != EOF)
	{
		...
		case 'I':
			clear_intfs();
			intf_count = parse_intfs(optarg);
			if(intf_count < 2){
				printf("need more arguments with -I\n");
				exit(-1);
			}
			break;
		...
		default:
			fprintf(stderr,
				...
				"-I                         multicast forwarding interfaces. (eth2.2 and br0 are by default) \n"
				"                           ex: -I \"br0 eth2.2\" \n"
				"                               -I \"br0 ra1 eth0 eth3:1 ra2\"\n"
#ifdef ECMH_BPF
...
#endif
				"-e, --enable_snooping      Enable MLD snooping for Ralink Switch\n"
				,
				argv[0]);
			fprintf(stderr,
	...
	if(g_conf->es && !g_conf->upstream){
		fprintf(stderr, "Error: please use \"-i\" to assign the upstream interface when enable MLD snooping.\n");
		return -1;
	}
	ralink_init();
