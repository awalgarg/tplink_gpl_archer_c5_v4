...
***************************************
 $Author: jianghua.qin $
 $Id: //WIFI_SOC/MP/SDK_5_0_0_0/RT288x_SDK/source/user/ecmh/src/interfaces.c#1 $
 $Date: 2015/03/04 $
**************************************/
...
int *getMTU(struct intnode  *intn)
{
	struct ifreq	ifreq;
	int		sock;
...
*/
	...
	/* Ralink */
#ifdef IGNORE_INTERFACE1
	if(g_conf->es && !strcmp(intn->name, IGNORE_INTERFACE1)){
		int_destroy(intn);
		close(sock);
		return NULL;		
	}
#endif
#ifdef IGNORE_INTERFACE2
	if(g_conf->es && !strcmp(intn->name, IGNORE_INTERFACE2)){
		int_destroy(intn);
		close(sock);
		return NULL;		
	}
