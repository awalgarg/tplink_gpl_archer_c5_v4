...
***************************************
 $Author: jianghua.qin $
 $Id: //WIFI_SOC/MP/SDK_5_0_0_0/RT288x_SDK/source/user/ecmh/src/ecmh_orig.c#1 $
 $Date: 2015/03/04 $
***************************************
...
*/
			...
			if (ifa->ifa_addr->sa_family == AF_INET6){
				memcpy(&addr, &((struct sockaddr_in6 *)ifa->ifa_addr)->sin6_addr, sizeof(addr));
			}else{
				memcpy(&addr, &((struct sockaddr_in *)ifa->ifa_addr)->sin_addr, sizeof(addr));
			}
...
#else
					(intn = int_create(ifindex, (IFF_POINTOPOINT == (ifa->ifa_flags & IFF_POINTOPOINT)) ? true : false)))
#endif
			{
				newintn = true;
			}
		...
		if (intn){
				...
				IN6_IS_ADDR_LINKLOCAL(&addr))
			{
				...
				if (ifa->ifa_addr->sa_family == AF_INET){
					...
					if (	gotipv4 && (ifa->ifa_flags & IFF_POINTOPOINT) != IFF_POINTOPOINT){
...
//dolog(LOG_DEBUG, "=>%02x %02x %02x %02x %02x %02x %02x %02x\n", sa.sll_addr[0], sa.sll_addr[1],sa.sll_addr[2],sa.sll_addr[3],sa.sll_addr[4],sa.sll_addr[5],sa.sll_addr[6],sa.sll_addr[7],sa.sll_addr[8]);
		/* Handle the packet */
...
int main(int argc, char *argv[])
{
	...
	/* Handle arguments */
	while ((i = getopt_long(argc, argv, "dfi:pPu:I:"
#ifdef ECMH_BPF
		"tT"
#endif
		"vV"
