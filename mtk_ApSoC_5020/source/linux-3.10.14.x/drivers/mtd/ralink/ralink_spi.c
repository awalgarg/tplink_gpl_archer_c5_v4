...
#ifndef CONFIG_REDUCE_FLASH_SIZE
#if defined CONFIG_MTD_PARTITION_SIZE_16M
#define TP_BOOT_SIZE 		0x20000
#define TP_KERNEL_SIZE 		0x200000
#define TP_ROOTFS_SIZE		0xda0000
#define TP_CONFIG_SIZE		0x10000
#define TP_ROMFILE_SIZE		0x10000
#define TP_ROM_SIZE			0x10000
#define TP_RADIO_SIZE		0x10000
#elif defined CONFIG_MTD_PARTITION_SIZE_8M
#define TP_BOOT_SIZE 		0x20000
#define TP_KERNEL_SIZE		0x200000
#define TP_ROOTFS_SIZE		0x5a0000
#define TP_CONFIG_SIZE		0x10000
#define TP_ROMFILE_SIZE		0x10000
#define TP_ROM_SIZE			0x10000
#define TP_RADIO_SIZE		0x10000
...
#else /* defined CONFIG_REDUCE_FLASH_SIZE */

#if defined CONFIG_MTD_PARTITION_SIZE_16M
#define TP_BOOT_SIZE 		0x10000
#define TP_KERNEL_SIZE 		0x200000
#define TP_ROOTFS_SIZE		0xdc0000
#define TP_CONFIG_SIZE		0x10000
#define TP_ROM_SIZE			0x10000
#define TP_RADIO_SIZE		0x10000
#elif defined CONFIG_MTD_PARTITION_SIZE_8M
#define TP_BOOT_SIZE 		0x10000
#define TP_KERNEL_SIZE		0x200000
#define TP_ROOTFS_SIZE		0x5c0000
#define TP_CONFIG_SIZE		0x10000
#define TP_ROM_SIZE			0x10000
#define TP_RADIO_SIZE		0x10000
#elif defined CONFIG_MTD_PARTITION_SIZE_4M
#define TP_BOOT_SIZE 		0x10000
#define TP_KERNEL_SIZE 		0x160000
#define TP_ROOTFS_SIZE		0x260000
#define TP_CONFIG_SIZE		0x10000
#define TP_ROM_SIZE			0x10000
#define TP_RADIO_SIZE		0x10000
#else
...
{
	struct mtd_partition* usrcfg_part=NULL;
	int i;

	for(i=0;i<sizeof(rt2880_partitions)/sizeof(struct mtd_partition);i++)
	{
		if(0==strcmp(rt2880_partitions[i].name, "config"))
		{
			usrcfg_part=&rt2880_partitions[i];
			break;
		}
	}
	...
	if(usrcfg_part->size>FLASH_MAX_RW_SIZE)
	{
		return -1;
	}
	
	if(spiflash_ioctl_read(l_buf, usrcfg_part->offset, usrcfg_part->size) < 0)
	{
		return -1;
	}
	...
	if(spiflash_ioctl_write(l_buf, usrcfg_part->offset, usrcfg_part->size) < 0)
	{
...
/* Deal ISP Config. Added by Wang Yang. 2016-12-15 */
#ifdef CONFIG_TP_CONFIG_RESET 

#ifndef CONFIG_REDUCE_FLASH_CONFIG
int spi_flash_erase_isp_config(void)
{
	if(spiflash_ioctl_read(l_buf, CONFIG_ISP_CONFIG_OFFSET, FLASH_MAX_RW_SIZE) < 0)
	{
		return -1;
	}
	...
	reset_write = 1;
#if	(defined(CONFIG_TP_MODEL_WR840NV4) && defined(CONFIG_TP_SPEC_BR_WISP))
	spiflash_ioctl_erase(CONFIG_ISP_CONFIG_OFFSET, FLASH_MAX_RW_SIZE * 2); // delete tag too, Wangyang 30Mar17.
#else
    //except TL-WR840Nv4 BR_WISP, the other now will erase one block. we will also erase option66 config file tag if needed.
	if(spiflash_ioctl_write(l_buf, CONFIG_ISP_CONFIG_OFFSET, 0x10000) < 0)  // isp config
	{
		memset(l_buf, '0', FLASH_MAX_RW_SIZE);
		reset_write = 0;
		return -1;
	}
#ifdef CONFIG_OPTION66_NEED_TAG
    memset(l_buf, '0', FLASH_MAX_RW_SIZE);
	if(spiflash_ioctl_read(l_buf, 0x7D0000, FLASH_MAX_RW_SIZE) < 0)
	{
		return -1;
	}
	
	spiflash_ioctl_erase(0x7D0000, FLASH_MAX_RW_SIZE);
	
	l_buf[0xF900] = 0x0; // ersase option66 config file tags
	spiflash_ioctl_write(l_buf, 0x7D0000, 0x10000);

    memset(l_buf, '0', 0x100);
#endif /* CONFIG_OPTION66_NEED_TAG */
#endif
	...
	l_buf[0xF800] = 0x0;     // ersase option66 config flag
	if (spiflash_ioctl_write(l_buf, 0x7D0000, 0x10000) < 0)
	{
		memset(l_buf, '0', FLASH_MAX_RW_SIZE);
		reset_write = 0;
		return -1;
	...
	if(spiflash_ioctl_write(l_buf, 0x7C0000, 0x10000) < 0)  // user config
	{
		memset(l_buf, '0', FLASH_MAX_RW_SIZE);
		reset_write = 0;
		return -1;
...
int spi_flash_erase_isp_config(void)
{
	struct mtd_partition* ispcfg_part=NULL;
	struct mtd_partition* usrcfg_part=NULL;
	struct mtd_partition* tag_part=NULL;
	
	int i;

	for(i=0;i<sizeof(rt2880_partitions)/sizeof(struct mtd_partition);i++)
	{
		if(0==strcmp(rt2880_partitions[i].name, "ispconfig"))
		{
			ispcfg_part=&rt2880_partitions[i];
		}
		else if(0==strcmp(rt2880_partitions[i].name, "config"))
		{
			usrcfg_part=&rt2880_partitions[i];
		}
		else if(0==strcmp(rt2880_partitions[i].name, "radio"))
		{
			tag_part=&rt2880_partitions[i];
		}
	}
	
	if(!ispcfg_part || !usrcfg_part ||!tag_part)
		return -1;

	if(ispcfg_part->size>FLASH_MAX_RW_SIZE || 
		usrcfg_part->size>FLASH_MAX_RW_SIZE ||
		tag_part->size>FLASH_MAX_RW_SIZE)
	{
		return -1;
	}

	if(spiflash_ioctl_read(l_buf, ispcfg_part->offset, ispcfg_part->size) < 0)
	{
		return -1;
	}
	...
	spiflash_ioctl_erase(CONFIG_ISP_CONFIG_OFFSET, FLASH_MAX_RW_SIZE * 2); // delete tag too, Wangyang 30Mar17.
#else
#error there is no 'romfile' partition any more
#endif
#else
    //except TL-WR840Nv4 BR_WISP, the other now will erase one block. we will also erase option66 config file tag if needed.
	if(spiflash_ioctl_write(l_buf, ispcfg_part->offset, ispcfg_part->size) < 0)  // isp config
	{
		memset(l_buf, '0', FLASH_MAX_RW_SIZE);
		reset_write = 0;
		return -1;
...
#ifdef CONFIG_OPTION66_NEED_TAG
    memset(l_buf, '0', FLASH_MAX_RW_SIZE);
	if(spiflash_ioctl_read(l_buf, tag_part->offset, tag_part->size) < 0)
	{
		return -1;
	}
	
	spiflash_ioctl_erase(tag_part->offset, tag_part->size);
	
	l_buf[0xF900] = 0x0; // ersase option66 config file tags
	spiflash_ioctl_write(l_buf, tag_part->offset, tag_part->size);
	...
	l_buf[0xF800] = 0x0;     // ersase option66 config flag
	if (spiflash_ioctl_write(l_buf, tag_part->offset, tag_part->size) < 0)
	{
		memset(l_buf, '0', FLASH_MAX_RW_SIZE);
		reset_write = 0;
		return -1;
	...
	if(spiflash_ioctl_write(l_buf, usrcfg_part->offset, usrcfg_part->size) < 0)  // user config
	{
		memset(l_buf, '0', FLASH_MAX_RW_SIZE);
		reset_write = 0;
		return -1;
...
EXPORT_SYMBOL(spi_flash_erase_isp_config);
 /* End Added by Wang Yang. 2016-12-15 */
