...
#if 0/*defined (CONFIG_RA_HW_NAT_PPTP_L2TP)*/
/*for FP*/
...
int lookup_chan_seq_send(u16 call_id, __be32 d_addr, u32** seq_send)
{
	struct pppox_sock *sock;
	struct pptp_opt *opt;
	int i;

	//printk("lookup_chan_seq_send call_id=%d, d_addr=%x\r\n", call_id, d_addr);
	rcu_read_lock();
	i = 1;
	for_each_set_bit_from(i, callid_bitmap, MAX_CALLID) {
		sock = rcu_dereference(callid_sock[i]);
		if (!sock)
			continue;
		opt = &sock->proto.pptp;
		/*printk("call_id=%d, dst_addr=%x\r\n", 
			opt->dst_addr.call_id, 
	...
	if(i < MAX_CALLID)
	{
		*seq_send = &(sock->proto.pptp.seq_sent);
		return 1;
	}
	return 0;
}
EXPORT_SYMBOL(lookup_chan_seq_send);
#endif
