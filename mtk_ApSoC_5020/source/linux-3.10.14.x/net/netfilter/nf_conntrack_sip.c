				    ...
				    unsigned int cseq)
{
	...
	spin_lock_bh(&nf_conntrack_lock);
	hlist_for_each_entry_safe(exp, tmp, &help->expectations, lnode) 
	{	
		if (exp->class != SIP_EXPECT_SIGNALLING ||
		    (exp->saved_addr.ip != daddr.ip) ||
	...
	if (nf_nat_sip_expect && ct->status & IPS_NAT_MASK)
	{
		...
		if(NF_ACCEPT == ret && exp)
		{
			addSipInfo(exp->tuple.dst.u3.ip, exp->tuple.dst.u.all, 
				exp->saved_addr.ip, exp->saved_proto.all, SIP_EXPECT_SIGNALLING_TIMEOUT);
		}
	}
	else {
		if (nf_ct_expect_related(exp) != 0) {
