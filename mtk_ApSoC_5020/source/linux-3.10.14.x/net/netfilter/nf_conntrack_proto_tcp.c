			  ...
			  u_int8_t pf)
{
	...
	pr_debug("tcp_in_window: I=%i II=%i III=%i IV=%i\n",
		 ...
		 before(sack, (receiver_offset >= 0 ? receiver->td_end: receiver->td_end - receiver_offset) + 1),
		 after(sack, receiver->td_end - MAXACKWINDOW(sender) - 1));

#if defined (CONFIG_RA_NAT_NONE)
	/* Changed by zhangwenkai, 2017.6.28, for Bug 173376
	 * here sack may have considerd NAT mangling while receiver->td_end has not,
	 * such that sack may > receiver->td_end
	 */
...
#else
		if (ack == receiver->td_end || ack == receiver->td_end - receiver_offset)
#endif
