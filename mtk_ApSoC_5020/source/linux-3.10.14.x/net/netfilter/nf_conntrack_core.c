...
/* Added by chz for new kernel 3.10.14. 2017-03-24 */
static size_t localConntractReadAddr(struct file *file, char __user *buf, size_t size, loff_t *ppos)
{
	int len = 0;
	char ret_buf[4];
...
static struct file_operations local_conn_fops = {
	.owner 		= THIS_MODULE,
	.read	 	= localConntractReadAddr,
	.write		= localConntractWriteAddr
};
/* end add */
...
int initLocalContractAddr(void)
{
	...
	procEntryAddr = proc_create("conntract_LocalAddr", 0644, init_net.proc_net, &local_conn_fops);
		...
		struct sk_buff *skb)
{
	...
	if (!ct) {
		...
		NF_CT_STAT_INC_ATOMIC(net, invalid);
		/* Changed by zhangwenkai, 2017.07.06, can't just goto out for IPV6 */
		if (PF_INET6 == pf)	
		{
	...
	if (ret <= 0) {
		...
		if( IS_SPACE_AVAILABLED(skb) &&
			...
			|| (FOE_MAGIC_TAG(skb) == FOE_MAGIC_L2TPPATH) 
#endif
		)){
			FOE_ALG(skb)=1;
		}
