...
#else
		/* Changed by zhangwenkai, 2017.7.3, for BUG 173997
		 * reason: while no multicast group lients, if received multicast packets,
		 * kernel need report the packet to imgp_proxy(user apps) more frequently to avoid long delay.
		 */
			     ...
			     struct sk_buff *pkt, vifi_t vifi, int assert)
{
	...
	/* Added by zhangwenkai , 2017.07.04, don't report data from LAN */
	if (strcmp(pkt->dev->name, "br0") == 0)
	{
		return 0;
	}
...
#else
	/* Changed by zhangwenkai, 2017.7.3, don't cache any packets
	 * reason: only cache 4 packets may cause image broken, 
	 *         and also more cost for we had changed expires timeout from 10s -> 0.5s
	 */
	kfree_skb(skb);
	err = 0;
#endif
...
#if 1 /* reset multicast ttl back to 64 */
