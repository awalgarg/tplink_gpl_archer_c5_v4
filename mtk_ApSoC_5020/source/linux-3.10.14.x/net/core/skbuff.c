...
static void kfree_skbmem(struct sk_buff *skb)
{
	...
	/* Added by chz. Moved from the end of this function. Set it before freeing the memory. 2017-08-10 */
...
bool skb_recycle_check(struct sk_buff *skb, int skb_size)
{
	...
	head_frag = skb->head_frag; /* keep head_frag, zhangwenkai, 2017.8.19 */
	memset(skb, 0, offsetof(struct sk_buff, tail));
	skb->pfmemalloc = pfmemalloc;
	skb->head_frag = head_frag; /* recover head_frag, zhangwenkai, 2017.8.19 */
...
int skbmgr_recycling_callback(struct sk_buff *skb)
{
	...
	unsigned long flags;
	__u32 rx0_length = skb_queue_len(&rx0_recycle);
        
	if (rx0_length < skbmgr_hot_list_len) {

                if ((skb->truesize != SKB_TRUESIZE(SKBMGR_RX_BUF_LEN)) ||
                        (skb_shinfo(skb)->nr_frags) ||
                        (skb_shinfo(skb)->frag_list)) {
                        return 0;
                }

                skbmgr_max_list_len = rx0_length + 1;
